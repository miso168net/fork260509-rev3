...
**階段**:**021-login-lockout-redis-cache ✅ 收刀（2026-06-28、硬化 019＝堵分散式打單帳號 DB 讀取/寫入放大、merge `5e4f3228`、feature branch 保留）；022-ip-access-control **SDD 設計鏈完成**（spec/clarify/plan/tasks/analyze ✅、待階段 2、feature branch 已建、詳 §6 marker）**；020-role-delete-policy-archive ✅ 收刀（2026-06-27、修 P-011-1 HIGH、merge `ebbd6c2c`、feature branch 保留）；019-login-lockout ✅ 收刀（2026-06-25、波 4 後獨立刀＝⚠️w 登入失敗節流落地、merge `dbc5902f`、feature branch 保留）；波 4 observability ✅ 全完成（2026-06-25）— 一刀 018-observability、三執行單元 obs-min→obs-full→dashboard（merge `c1a3224`、feature branch 保留）＝完全 opt-in 維運觀測層〔loki/alloy/grafana log＋prometheus/exporter/pushgateway metrics＋6 dashboard＋3 alert〕、rust 埋点 RUSTAPI-SOURCE-ISOLATION〔json log+trace_id 關聯／/metrics+casbin counter／cleanup push〕、零 base-web/零 migration/零新 crate、MSRV 1.86 確證。波 3 ✅（014/015/016）＋pre-波4 017 ✅（merge `c7f5936`）＋波 2 ✅（009-012）＋D11 013 ✅ 已收**（as-built 帳見 [DECISIONS §2](INTEGRATION-DECISIONS.md)）

**最新進展**(滾動最近 2 條;完整歷史見 [`docs/INTEGRATION-MILESTONES.md`](INTEGRATION-MILESTONES.md)):
- **2026-06-28 022-ip-access-control SDD 設計鏈完成（brainstorm→spec→clarify→plan→tasks→analyze→remediation、待階段 2）**：通用 IP 白/黑名單存取閘——全站每請求對 real_ip 比對 CIDR（白>黑>default-allow）、DB 真相→ArcSwap 記憶體判定→redis invalidate 門鈴、fail-OPEN、admin 自鎖防護+手動解鎖（reset-marker per-dim）、CF Tunnel 繞 nginx 拓樸正式化+013 real_ip 窄 fallback。spec 16/16 quality、clarify 0 ambiguity、plan Constitution 9/9、tasks 38/4 執行單元、analyze **0 CRITICAL/HIGH**（2 獨立 reviewer 接住 **I1**＝tunnel-fallback 改新 pure fn `apply_tunnel_fallback`〔不動 resolve_client_ip 簽名〕+ G1/G2/I2/D1/D2/T1、二輪 remediation 全修）。1 migration m007（破連 4 刀 0-migration streak）/6 route/0 新 crate（arc-swap 提升）。commit 鏈 brainstorm `b22485f0`→spec `d92cf78f`→plan `7e296fb7`→tasks `e4fcd90e`→remediation `90c049bf`；specs=[specs/022-ip-access-control/]
...
**下一步**: **022-ip-access-control SDD 設計鏈完成 → 階段 2** `superpowers:executing-plans` + Workflow 驅動 TDD（貼 §3 驅動提示詞、branch=022-ip-access-control）。4 執行單元 U1-U4（**pin bump 依單元邊界非 Phase**）、設計接地見 specs/022-ip-access-control/{tasks,plan,research,contracts}（詳 §6 marker）。019／020／021／波4 均已收刀（見 §2／MILESTONES）。下一波 roadmap 見 [DESIGN §8.4](INTEGRATION-DESIGN.md);候選遞延項＝alt-login 4 流程 stub〔⚠️m post-波3 v1-completeness slot、§4.2〕／audit scale 兩項〔pg_trgm／archive purge、§4.2〕／log retention purge〔⚠️n、§4.2〕。018 自身遞延 backlog 見 §3.I。Auth 島／治理島 §4.2／三維 RBAC runtime 編輯／觀測層皆已閉口
