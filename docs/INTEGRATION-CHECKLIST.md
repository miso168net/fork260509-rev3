...
- **2026-06-27 020-role-delete-policy-archive 全綠收刀（修 P-011-1 HIGH）**：角色軟刪（單筆/批次）同交易 archive-move 該 code **全維** casbin 授權（reason=`role_soft_delete`）+ 移除 → 重建同 code **0 繼承**；三條繼承路徑全封——getMenu（archive-on-delete）／回收桶 restore（讀時 `created_at` 衍生 restorable、零 mutation/零 migration）／**grant-during-delete FOR-UPDATE 鎖序（user 拍板 A）**；restorePolicy 拒→2222 `biz.policy.notRestorable`。3 執行單元 Workflow 驅動（U1 rust archive-on-delete+衍生/守門 `46b2941`..`3c65d17`／U1b grant-lock `67ff2bd`／U2 base-web 回收桶條件鈕+i18n `fec0a6d`、pins outer `1e72d3fb`/`eafcbe90`）。rust 226 測+curl C-V-1/2/5/6/7/8+CDP C-V-9+holistic 冷讀全綠、0 migration/crate/route、Constitution 9/9。merge `ebbd6c2c`、feature branch 保留。詳 [DECISIONS §1 P-011-1](INTEGRATION-DECISIONS.md)

> 以下為預計`下一步` (不要合到`最新進展`)

**下一步**: **當前無 active feature（021-login-lockout-redis-cache 已於 2026-06-28 收刀、merge `5e4f3228`、feature branch 保留）**。019／020／波4 等均已收刀（見 §2／MILESTONES）。下一波 roadmap 見 [DESIGN §8.4](INTEGRATION-DESIGN.md);候選遞延項＝alt-login 4 流程 stub〔⚠️m post-波3 v1-completeness slot、§4.2〕／audit scale 兩項〔pg_trgm／archive purge、§4.2〕／log retention purge〔⚠️n、§4.2〕。018 自身遞延 backlog 見 §3.I。Auth 島／治理島 §4.2／三維 RBAC runtime 編輯／觀測層皆已閉口
