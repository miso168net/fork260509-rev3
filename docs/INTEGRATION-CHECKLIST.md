...
**階段**:**024-password-policy ✅ 收刀（2026-07-02、密碼複雜度政策：admin 於 system-settings 設 7 政策〔m009 seed 2 number+5 enum+number 型驗證整串比對〕+`password_policy` dormant 純驗證原語〔供刀2 025-user-center〕+收尾 UI〔滾動修復/分區塊密碼策略·會話設定/2 欄緊湊 grid/區塊標題 i18n〕、final holistic review 全 PASS、merge `aca02c81`、feature branch 保留）；023-list-column-sort ✅ 收刀（2026-07-01、列表欄位排序＝7 頁 server-side 多欄排序+一鍵清除+localStorage per-route 持久化、final holistic review PASS、merge `009901fa`、feature branch 保留）**；022-ip-access-control ✅ 收刀（2026-06-29、通用 IP 白/黑名單存取控制閘＝全站 `ipgate_mw` 白>黑>default-allow＋CF Tunnel real_ip 窄 fallback＋手動解鎖 per-dim、final holistic review 3/3 SHIP、merge `d9c809e6`、feature branch 保留）；021-login-lockout-redis-cache ✅ 收刀（2026-06-28、硬化 019、merge `5e4f3228`、feature branch 保留）；020-role-delete-policy-archive ✅ 收刀（2026-06-27、修 P-011-1 HIGH、merge `ebbd6c2c`、feature branch 保留）；019-login-lockout ✅ 收刀（2026-06-25、波 4 後獨立刀＝⚠️w 登入失敗節流落地、merge `dbc5902f`、feature branch 保留）；波 4 observability ✅ 全完成（2026-06-25）— 一刀 018-observability、三執行單元 obs-min→obs-full→dashboard（merge `c1a3224`、feature branch 保留）＝完全 opt-in 維運觀測層〔loki/alloy/grafana log＋prometheus/exporter/pushgateway metrics＋6 dashboard＋3 alert〕、rust 埋点 RUSTAPI-SOURCE-ISOLATION〔json log+trace_id 關聯／/metrics+casbin counter／cleanup push〕、零 base-web/零 migration/零新 crate、MSRV 1.86 確證。波 3 ✅（014/015/016）＋pre-波4 017 ✅（merge `c7f5936`）＋波 2 ✅（009-012）＋D11 013 ✅ 已收**（as-built 帳見 [DECISIONS §2](INTEGRATION-DECISIONS.md)）
...
- **2026-07-01 023-list-column-sort 全綠收刀（列表欄位排序：多欄·伺服端·可保留；merge `009901fa`、feature branch 保留）**：7 個分頁列表（user/role + 5 審計/治理頁、menu 排除）點欄頭 **server-side 多欄排序**〔點擊序＝優先序、naive-ui 原生 3-state〕＋一鍵清除＋localStorage per-route 持久化。後端 `parse_sort_spec`＋per-entity `resolve_sort` `match` 白名單〔防注入、relocate facade 模組守 entity_access_lint〕＋7 facade `list` sort apply〔空→byte-identical 現況、非空→order_by＋Id tie-break、ip_rule 保留 `deleted_at IS NULL` 領頭群組 F4〕＋7 handler 接線；export 共用 facade→CSV 自動反映排序（FR-016、前端匯出鈕亦帶 sort）；**m008** 補 `idx_login_attempt_created_at`（clarify Q1、唯一資料層改動）。前端 `useTableSort`〔受控排序＋自維護點擊序 reconcile＋wire 字串＋`clearAll`＋per-route localStorage 持久化＋FR-015 還原防禦丟棄失效欄；純函式抽 `use-table-sort-core.ts`〕＋`SortClearButton`〔7 頁掛載：3 頁 `#suffix`／4 表既有 `NSpace`、不改 `table-header-operation.vue` 本體〕＋`WithSort<T>` typings＋i18n〔`common.clearSort`＋新建 `backend.biz.common.invalidSort`、Schema-first〕；audit 3 表複合 storageKey `route.name:tab`（F3 不互污染）。U1-U4 執行單元＋U5 全量驗證/final-review、Workflow 驅動 implementer TDD→spec∥quality review→fix loop＋主線逐單元邊界獨立 ground-truth 自驗（git show --stat＋容器內 build/test＋curl/CDP live＋bump pin S9）；rust 235 unit＋lint(2+2)＋migration up→down→up＋base-web typecheck＋tsx 22 斷言＋各單元 CDP（3-state·多欄點擊序·分頁重置·無 badge FR-014·衍生欄不可排 FR-012·清除·持久化·FR-015·audit per-tab）全綠、curl 7 端點空 sort 等同預設（SC-006 零回歸）·非法 2222·匯出反映排序、**final holistic review fresh-agent 冷讀 PASS**（FR-001~016/SC-001~007 全覆蓋、三端對齊零漂移、Constitution 9/9）、0 新 crate（1 index-only m008）、MODAL-WIRING (f) v1.2.0。pins rust-api `2591efe`／base-web `78791b33`。詳 [DECISIONS §1 ⚠️af](INTEGRATION-DECISIONS.md)

> 以下為預計`下一步` (不要合到`最新進展`)

**下一步**: **下一刀待定（當前無 active feature）**。024-password-policy 已收刀（merge `aca02c81`、2026-07-02、密碼複雜度政策）；**刀2 候選＝025-user-center**（個人中心、接 024 `password_policy` 原語 enforce 改密碼、rev3 sys_user profile 欄已存·缺 self-service 端點）。下一刀候選見 [DESIGN §8.4 roadmap](INTEGRATION-DESIGN.md);遞延項＝alt-login 4 流程 stub〔⚠️m post-波3 v1-completeness slot、§4.2〕／audit scale 兩項〔pg_trgm／archive purge、§4.2〕／log retention purge〔⚠️n、§4.2〕／帳號-DoS 緩解〔**IP 信任白名單部分已由 022 白名單兌現**、CAPTCHA 仍 future、019 §4.2〕／**未來 zh-TW locale feature**〔建 zh-tw.ts+語言切換「中文」改「簡體」加「繁體」；024 刻意留簡繁混用待此處理〕。018 自身遞延 backlog 見 §3.I。Auth 島／治理島 §4.2／三維 RBAC runtime 編輯／觀測層／IP 存取控制閘（022）／列表欄位排序（023）／**密碼複雜度政策（024）**皆已閉口
