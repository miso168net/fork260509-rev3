...
- **2026-07-02 REVIEW-20260702 §3 統一修復輪完成（F-1~F-11 全數處置；rust `77b8d18`/outer `09ea097e`、base-web `0626ead6`/outer `b5d0b35b`）**：直接修 5〔F-1 user-center 註解對齊 as-built／F-2 `find_all` ORDER BY setting_key（消 UI 位置漂移、live 序驗）／F-3 CLAUDE.md §8.1 sys_user **16 欄實況**／F-4 user+role 搜索重置**自動重查**（user 拍板＝對齊全站其餘 5 頁慣例、CDP 兩頁 PASS）／F-6 userCenter self 並發硬刪 no-op→`biz.user.notFound` 不假報成功（user 拍板）〕＋F-5 不修（user 拍板維持 upstream 原樣）＋登記 4（F-7→§4.2、F-8/F-9→§3.H、F-10→§3.I）＋F-11 017 spec FR-005a 勘誤。rust 255 unit＋lint(2+2) 零回歸；base-web typecheck 綠。詳 [REVIEW-20260702 §3](REVIEW-20260702.md)
...
- [ ] **casbin deny 序列未 pre-register ＋ pushgateway 無持久卷**〔018、low/obs 慣例級〕（REVIEW-20260702 F-10）：`casbin_enforce_total{decision="deny"}` 未 pre-register 0——重啟後至首 deny 前該序列不存在、dashboard 空態非 0（`or vector(0)` 已緩解 alert 面）；pushgateway 無持久卷（重啟失憶至下次 cleanup push、exporter 無狀態不受影響）。觸發＝下次動 metrics 埋点時 pre-register 0／評 pushgateway named volume。
...
- [ ] **009/024 初始密碼政策化（future feature 候選）**〔REVIEW-20260702 F-7〕：addUser 初始密碼寫死 `"123456"`（`system_manage.rs`）、不經 024 `password_policy` 驗證——admin 設嚴政策後、新帳號初始密碼即不合政策（符合 009 假設＋024/025 scope、非缺陷）。候選修向＝初始密碼政策化（可配置/隨機生成）＋強迫首登改密（接 025 改密 enforce 既有原語）。
