...
- **2026-06-28 022-ip-access-control Phase 0 brainstorm ✅ 定版（commit `b22485f0`、待手動 `/speckit-specify`）**：通用 IP 白/黑名單存取閘——全站每請求對 real_ip 比對 CIDR（白>黑>default-allow）、DB 真相→ArcSwap 記憶體判定→redis invalidate 門鈴、fail-OPEN、admin 自鎖防護、手動解鎖（reset-marker per-dim）、CF Tunnel 繞 nginx 拓樸正式化+013 real_ip 窄 fallback 強化。兌現 019 §4.2 per-IP 白名單 + 021 FR-012 L0 seam；獨立 cold-review 接住 1 HIGH（CF-CIP 信任面收窄）+3 MED 已修進 doc。brainstorm=[docs/superpowers/022-ip-access-control.md]
...
**下一步**: **022-ip-access-control brainstorm ✅ 定版（2026-06-28、commit `b22485f0`）→ 待【手動】`/speckit-specify`**（input=`docs/superpowers/022-ip-access-control.md`、起 022 feature branch；★ 勿排進 brainstorm 觸發、否則 `speckit.git.feature` pre-hook 不跑、§3）。019／020／021／波4 均已收刀（見 §2／MILESTONES）。下一波 roadmap 見 [DESIGN §8.4](INTEGRATION-DESIGN.md);候選遞延項＝alt-login 4 流程 stub〔⚠️m post-波3 v1-completeness slot、§4.2〕／audit scale 兩項〔pg_trgm／archive purge、§4.2〕／log retention purge〔⚠️n、§4.2〕。018 自身遞延 backlog 見 §3.I。Auth 島／治理島 §4.2／三維 RBAC runtime 編輯／觀測層皆已閉口
