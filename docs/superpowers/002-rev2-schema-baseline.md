...
- **活庫實證**（rev2-admin-postgres-1、pg_dump 17.10 schema dump 717 行）：**零 FK**（含 sys_user_role）；partial unique ×3（`WHERE deleted_at IS NULL`）；一般 index 7；audit archetype 四變體分布與 DESIGN §3.2／constitution §I.6 一致。
...
- **argon2 seed**：runtime 生成（`SaltString::generate`＋`Argon2::default()`、plaintext `123456`、3 帳號共用單次 hash）；PHC 字串 SQL-literal 安全（rev2 m002 注記）。
...
- 忠實基線：零 FK、partial unique ×3、一般 index 7、audit archetype 照活庫、INET `custom(Alias)`、`unique_key_sea_orm_adapter` 由 adapter 建（CONSTRAINT 形自動對齊）
- sys_user 直接 BIGSERIAL 建表（rev2 retrofit 鏈不重演）；對稱 `down()`（波 0 出口 up→down→up 守恆）

**§3.3 m002_rev2_seeds（92 列／6 表淨效果）**
- 直接 INSERT 終態值（UPDATE 回填鏈不重演、淨值入 INSERT）；§3.4 紀律②：sequence-driven 插入（空表確定性落 id 1/2/3）、user_role／casbin 引用 subquery 解析；argon2id runtime hash；冪等 `ON CONFLICT DO NOTHING`
- 終態斷言數字：casbin protected=true **19** 列、sys_menu protected=true **8** 列

**§3.4 m003＋m004（rev3 delta 顯式分離）**
- m003：`sys_user_role` FK ×2（→sys_user.id／sys_role.id；④拍板）
...
2. 雙 dump（容器內 pg_dump 17.10）→ normalize（`\restrict` 過濾／排除 seaql_migrations／argon2 hash・timestamps・setval 正規化）→ **schema diff＋6 表 seed data diff 雙零差異**
3. up→down→up 守恆＋dev stack migrate gate 實機（m001~m004 一次 up、rust-api healthy）＋活庫 sanity＋prod image build（adapter COPY 驗）＋lock 變動後 time 複驗

**§3.6 已知坑（spec 必載）**
- sys_user sequence 終態對齊（pristine 庫 setval=3/is_called 與 rev3 sequence-driven 結果互驗；data dump 的 `setval` 行入 normalize）
- m002 manage 樹 menu policy（10 列集合）與 m004 demo 擴充集**不重疊**邊界
- adapter 拷入後 `cargo build`＋Dockerfile prod build 雙驗；xdb 不入本刀（[[bench]] 坑屬 audit 刀）
