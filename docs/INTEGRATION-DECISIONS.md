...
| 待決③ | 縱切第一刀：User 直刀 vs `system_settings` 打樣 | ✅ 已決(2026-06-15)：**A — User 直刀**。`sys_user` 為 login/`operator_id`/roles 根 entity、最早凍結收益最大；一刀逼出 §5.1-5.4+§5.8 全套（password redact＋分頁/filter）+M:N join（`sys_user_role` replace_roles_in_txn）+argon2 預設密碼+停用登入 gate+★L4 MODAL-WIRING 前端軌。**B（system_settings 打樣）被否**：其「便宜排練 §8.1 全管線」核心價值已由波 0 七刀實戰驗證 7 次而大幅縮水，換不回漏掉的 §5.8/join/id-string 暴露（varchar PK 無代表性、那些面拖第二刀才暴露）。代價接受＝第一刀即最重（rev2 016+017 規模）。⇒ 波 1 第一刀＝User 刀。user 親決 | §8.3 | ✅ 已決 |
...
| ⚠️o | application-RI 驗證下沉 facade 層 | ✅ 已決(2026-06-15)：**維持 handler 層驗**（不下沉 facade）。沿 rev2 §3.3——grant 前 handler 解 v0 code（`find_active_by_id`→查無 2222）再呼 facade；facade 收 `role_code:&str` 不驗；reparent 3+1 guard 等 RI 檢查本需 handler context。下沉 facade 屬設計變更、本刀不採；波 1 User 刀 facade 沿此。user 親決 | §3.3 | ✅ 已決 |
...
| ⚠️u | constitution §IV 缺 §I.4 工作流對應題（**amendment 提案**） | ✅ 已決(2026-06-15)：**不採納、日後不再議**——維持 constitution §IV 現 9 題、不增第 10 題（「plan/tasks 是否把 push/merge 排入實作期」）、不 bump 1.0.1。原提案(2026-06-13)背景：001 `/speckit-analyze` 發現 T021 曾排 worktree push、現 9 題抓不到。user 親決不立此 Check（§I.4 凍結令＋人工 review 已足、避免 Check 膨脹）；本項 close、不再重議 | constitution §IV | ✅ 已決 |
...
| ⚠️x | endpoint_coverage_lint 是否為波 0 出口硬條件（三守恆之一） | ✅ 已決(2026-06-14)：**波 0 換波豁免**——`endpoint_coverage_lint`（鎖 main.rs router==ENDPOINT_REGISTRY==seed、`EXPECTED_ROUTE_COUNT=35` gated route，DESIGN §7.1／§8.4）**結構上需首個受保護業務端點存在才能立**；波 0（001-007 地基）**零 gated 業務端點**、無法在波 0 達成。故 user 親決：波 0 出口三守恆**豁免** endpoint_coverage_lint、**移交波 1 第一刀**（首個 gated endpoint wiring 時一併立、enforce_mw route_layer 首次真掛載，§3.9／§3.6）。波 0 餘二守恆（entity_access_lint ✅ 004／migration up→down→up ✅ 002）＋另三出口全綠 → **波 0 正當收官**。（前 brainstorm v1〔已 revert、commit `be3eea6` 不在任何分支〕誤以 ⚠️w 暫記此豁免；⚠️w 正解＝login lockout、本碼 ⚠️x 為正式登記） | §8.4・§3.9 | ✅ 已決 |
...
rev2 001-012+015 對應的跨切地基；首個 spec-kit feature 序列（七刀）。
...
- **★ CDP 抓修真 bug**（`0de38d6`）：前端 axios 把未設 filter 序列化成空字串 query（`status=`）→serde `Some("")`→`wire_to_i16` 回 2222／`user_phone=''` 排除 NULL-phone→**列表瀏覽器空列**；修＝空字串視為未設（FR-002 空欄略過）。**curl 乾淨 query 5+ 輪全綠完全掩蓋、CDP browser modal smoke 才抓到＝「curl≠modal、雙軌驗收」價值印證**（C-V-6 不 defer 的回報）。見 CLAUDE.md §3 Phase 0（list filter 空字串守門）。
