...
- [ ] T007 [US2] **test-first**：`rust-api/server/src/handler/system_settings.rs` 的 `#[cfg(test)] mod value_type_tests` 內加 number case（用 **bare `"number"`** value_type，沿既有 `.expect_err` + `err.code()` + `err.key().as_ref()` 風格）：`validate_value_type("number", "12"/"1"/"1024")` → `is_ok()`；`"abc"/"0"/"-1"/"1025"/"9999"/""` → `err.code()=="2222"` 且 `err.key().as_ref()=="biz.systemSettings.invalidValue"`（含 `"1025"`＝max+1 上界邊界）。**先寫、預期紅**（number 目前落 `_ => Ok(())` passthrough、非法值誤 pass）。
