...
| enforce-gated route(Bearer) | next（200） | 無 policy→`permission_denied`(code `"5003"`、HTTP 403)；token 缺/失效→`token_expired`(`"3333"`)；**role-lookup DB error→fail-closed `permission_denied`(5003)＋log**（R6/DESIGN §10.1） |
