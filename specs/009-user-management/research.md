...
> ⚠️ **校正（009 U1 活體 2026-06-18）**：下方 Decision 的 `PgExpr::ilike(LikeExpr::new(...).escape(反斜線))` 處方【編譯過、runtime 失效】——sea-query 0.32.7 的 `drop_right_escape_hack` 只認 `BinOper::Like`（不含 pg 專屬 `PgBinOper::ILike`），ILIKE+ESCAPE 渲染成非法 `ILIKE (pattern ESCAPE char)`（postgres syntax error→整頁 5000）。**實作改用** `LOWER(col) LIKE '<pattern>' ESCAPE 反斜線`（`Expr::expr(Func::lower(Expr::col(col))).like(LikeExpr::new(...).escape(...))` ＋ column/pattern 兩端 lowercase 達大小寫不敏感；escape_like wildcard 規則不變）。權威實作＝rust-api `server/src/model/facade/sys_user.rs` 的 `ilike()` helper header。下方原處方保留為研究軌跡、勿照抄。curl 乾淨 query 不帶 escape→不撞；帶 filter 即爆＝§5.8「curl≠modal」經典案例。
