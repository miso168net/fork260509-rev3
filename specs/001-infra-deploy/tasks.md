...
- [x] T013 [US1] dev 實機驗收：C-V-0（standalone down＋secrets/cert 就緒）→ C-V-2 全套（`up -d --wait` exit 0／ps 5 healthy＋migrate exited(0)／gate 時序雙斷言／健檢 6 點含 Content-Type）→ C-V-8（缺 secrets fail-fast 負向＋重複 up 冪等；Edge case 2 豁免註記確認）
- [x] T014 [US1] 持久化驗收 C-V-4（probe 表寫入→down→up→存活→清除）
