...
- **FR-003**: 系統 MUST 提供零漂移驗證流程——於**基線檢查點**（delta 套用前）與前代純淨參考庫（由前代既有資產重放產生）做結構＋6 表種子資料雙向比對（「雙向」＝無缺漏亦無多出，由 diff 單次對稱比對涵蓋、非兩輪），正規化後零差異；正規化規則明定（隨機 token 行、框架追蹤表內容、密碼雜湊、時間戳、序列狀態值）
