...
- [ ] T001 整檔拷貝 `rust-api/sea-orm-adapter/`（自 `fork260509-rev2/rust-api/sea-orm-adapter/`：Cargo.toml＋src/{lib,adapter,entity,action,migration}.rs＋examples/×4——§I.5 例外、byte-identical 帶入＋檔頭拷貝來源注記核對）；`rust-api/Cargo.toml` members 加 `"sea-orm-adapter"`
- [ ] T002 workspace deps 增量（`rust-api/Cargo.toml`）：`casbin = { version = "2.20", default-features = false }`＋`sea-orm = { version = "1.1.20", default-features = false, features = ["macros", "sqlx-postgres", "runtime-tokio-rustls"] }`（**R3 宣告形——刻意偏離 rev2 defaults-on、time 不入圖、注記寫進檔**）＋`argon2 = "0.5.3"`；`rust-api/migration/Cargo.toml` 加 `argon2.workspace = true`＋`sea-orm-adapter = { path = "../sea-orm-adapter" }`
...
- [ ] T004 [P] `deploy/Dockerfile.rust-api.txt` Manifest／Source 段補 sea-orm-adapter COPY ×2 行（照檔內教訓註解格式；outer 單段 commit）
...
- [ ] T008 [US1] `tests/002-rev2-schema-baseline/scripts/`：`pristine-replay.sh`（cv002-net＋pg＋rev2 image 35 支重放）＋`normalize.sh`（**migration-chain.md §3 五規則凍結形**）＋`diff-baseline.sh`（rev3 拋棄式 pg＋`up -n 2` 檢查點＋雙 dump＋雙 diff＋計數不變式＋argon2 VERIFY）
- [ ] T009 [US1] C-V-2 實機：pristine 重放（seaql 35 applied 斷言）＋normalize 後基準 dump 產出→`tests/002-rev2-schema-baseline/rev2-schema-baseline.sql`（git-track）
...
- [ ] T011 [US2] `rust-api/migration/src/m003_user_role_fk.rs`：FK ×2（→sys_user.id／sys_role.id、`ON DELETE RESTRICT`）＋down 卸 constraint；`lib.rs` 掛載
- [ ] T012 [US2] `rust-api/migration/src/m004_demo_menu_seeds.rs`：demo 選單 66 列（**轉錄權威＝contracts/demo-menu-enumeration.md、逐列回 route 源檔親驗 meta**；深度 4 段分批、parent_id subquery；document 8 頁 href 化〔R4-D2〕；plugin menu_name 中文；欄位映射＝research R4）＋menu policy 66 列（`'p','R_SUPER',<route_name>,'menu','','',''`、全覆蓋〔R4-D3〕）＋**down 限定 demo 66 route_name 集**（不得波及基線——migration-chain.md 明文）；`lib.rs` 掛載
- [ ] T013 [US2] `scripts/delta-assert.sh`＋C-V-4 實機：FK=2（confdeltype='r'）／sys_menu 76／casbin 138（menu 維度 83）／基線 92 列原值不變＋protected 19・8 不變
...
- [ ] T015 [US3] C-V-6 實機：再次 `up` exit 0、無 pending、6＋2 表列數全不變（SC-008）
...
- [ ] T018 C-V-9 殘留 grep（修訂規則：部署層零豁免不變；rust-api 側 `m00*_rev2_*`＋migration README＋adapter 檔頭注記豁免）＋quickstart.md 流程逐步對照零漂移＋拋棄式資源清理（cv002-*）
...
- Phase 4：T011∥T012（不同 migration 檔；lib.rs 掛載序固定 m003→m004）
