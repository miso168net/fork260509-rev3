...
- [ ] T010 [US1] live button（in-crate `#[ignore]`、容器內 `--test-threads=1`、`DATABASE_URL`、trace_id `"016-button-smoke"`、snapshot-restore guard 自清）in `sys_casbin_rule.rs`（或 archive test mod）：updateRoleButton 撤一非保護 code→psql casbin v2='button' 列無＋archive 有（archive_reason='role_dimension_revoke'）／grant→新列／restore→回＋archive 消費＋審計／`button_codes_for_role` 反映；**★ teardown 明列清自身 casbin op-log（`DELETE sys_operation_log WHERE entity_table='casbin_rule' AND trace_id='016-button-smoke'`、鏡像 015 archive_restore／011:302-308、否則破 012 `t_cas==2` baseline〔H1〕）**（C-V-2；依 T005）
...
- [ ] T014 [US2] main.rs 註冊 3 endpoint route（require_policy R_SUPER、m002 seed）in `rust-api/server/src/main.rs` + **`AS_BUILT_ROUTES` 40→43** + **registry 防漂移 assertion〔雙向、M2〕**（`ALL_ENDPOINT_POLICIES` == registered policy-governed endpoint：⊇ 防漏端點〔UI 授不到〕＋ ⊆ 防 stale phantom〔UI 幻影端點→grant 出無 route 消費的孤兒 casbin 列〕；若保留「可授權但暫未掛 route」者須明列白名單例外）in `rust-api/server/tests/endpoint_coverage_lint.rs`（D3；依 T013/T012）
...
- [ ] T019 [US3] 回收桶三維 v2-推導〔**★ 同改兩處、L4**〕：(a) `getArchivedPolicies` 回應 `dimension` 由 v2 推導（`match v2 {"menu"|"button"=>原值, _=>"endpoint"}`、display 層）+ (b) list facade `?dimension=endpoint` filter→`v2 IN HTTP_METHODS`（現 list 是直接 `V2.eq(d)`、須加 endpoint 特例）in `rust-api/server/src/handler/system_manage.rs` + `rust-api/server/src/model/facade/sys_casbin_policy_archive.rs`（D3；**不改 archive_reason**、依 T002；依 015 既有 list/restore）。**★ M3：restore 審計 payload `dimension` 欄記 raw v2〔endpoint=method、保 (path,method) 完整 forensic 識別〕、與回收桶 display 推導刻意不同＝by-design（見 data-model §4）、不對齊**
- [ ] T020 [US3] live gate 精準（in-crate `#[ignore]`、`--test-threads=1`）：updateRoleButton/Endpoints Rejected→**不** `load_policy` **不** publish〔publish 計數=0〕／Applied（含**空-diff** 送相同集合）→**必** reload+publish（鏡像 015 gate 測法、復用 reload_and_publish）in `sys_casbin_rule.rs`；**★ teardown 明列清自身 casbin op-log（trace 隔離、否則破 012 `t_cas==2` baseline〔H1〕）**（C-V-4；依 T005/T013）
- [ ] T021 [US3] live 2-instance（`$DC --profile multi up -d rust-api-2 --wait`、:31082、shared pg/redis）：副本 A updateRoleButton 撤某 button→psql 證→副本 B `getUserInfo` buttons 反映（watcher 收斂）／A updateRoleEndpoints 撤某 endpoint→B 對該 (role,path,method) enforce 被拒／A restore→B 恢復／`CLIENT KILL TYPE pubsub`→重訂閱仍收斂；**id-snapshot casbin op-log 清理（保 012 `t_cas==2` baseline〔H1〕）**＋`--profile multi down`（C-V-6；依 T011/T005、watcher 復用 015〔T002 endpoint 基元〕；★ L1 typo 修：原誤寫自我參照 T021）
...
- [ ] T025 零回歸 + prod build：`--ignored` 全套綠（menu 011/015 + button/endpoint 新測；archive_reason menu/button vs endpoint 並存）；**★ H1 baseline 具體斷言：`audit_query_oplog…`〔`sys_operation_log.rs:207`〕的 `entity_table~casbin` 仍恰 2 列（016 live teardown 已清自身 casbin op-log、`t_cas==2` 不破；sweep 順序不影響）**／非-ignored 全套綠／`git diff migration/` 空＋up→down→up 綠／base-web `pnpm typecheck`／**prod target image build**（`docker compose -f docker-compose.yml -f docker-compose.prod.yml build rust-api`）（C-V-8；依全部）
...
```
- **rust 全程 serial**（rust 任務 T002-T006／T010-T014／T018-T021 全 serial、共用 crate、即使不同檔不平行 cargo；base-web 任務 T007-T009／T015-T017／T022 不在此列、屬跨棧可平行〔L3 釐清〕）。
