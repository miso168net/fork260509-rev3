...
- `archive_reason` 取值（本刀後全集）：`role_dimension_revoke`（menu/button 手動撤銷）／`role_button_revoke`（保留相容）／`role_endpoint_revoke`（endpoint 手動撤銷）／**`role_soft_delete`（本刀新增＝角色刪除歸檔其 active 授權）**。
...
```
restorable(r) :=
    r.archive_reason ∈ {role_dimension_revoke, role_button_revoke, role_endpoint_revoke}
    AND  ∃ role ∈ sys_role : role.code = r.v0
                              AND role.deleted_at IS NULL          (active 角色存在)
                              AND role.created_at < r.archived_at  (歸檔屬【當前】角色實例)
```

- 條件一（reason 為撤銷類）：排除 `role_soft_delete` 列（角色刪除歸檔列永不可復原）。
...
2. （同一 `mutate_in_txn`）soft_delete 角色：設 `deleted_at/by` + SoftDelete op-log（既有）。
...
### restorePolicy（擴充守門）
- 載目標 archive 列 → 套 restorability 規則：不可復原 → **2222 `biz.policy.notRestorable`**（新增 `RestoreOutcome::NotRestorable`）；可復原 → 既有 Applied/NoOp/NotFound 流程不變。
