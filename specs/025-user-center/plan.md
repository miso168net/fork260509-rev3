...
把 `/user-center` 的 `<LookForward/>` 佔位換成真個人中心：**4 區塊頁**（基本资料〔含 created/updated 唯讀列〕/手机/邮箱/改密码，naive `NForm`+`NCard`、版面複刻 `form/basic`＋`function/request`）＋**4 auth-only self 端點**〔getProfile/updateProfile/changePassword＋〔U2 as-built〕**getPasswordPolicy**、operator=`claims.uid`、免 casbin〕＋**2 支窄寫 facade fn**（`update_own_profile` 寫 gender/nick/phone/email、`change_own_password` 寫 password；皆 §I.6 成對＋op-log redact）＋**喚醒 024 dormant `password_policy`**（改密碼 `find_all→from_settings→validate_password_complexity(&policy,新密,user_name)→verify(舊密)→hash→窄寫`）。前端動態密碼 rule 取自 **`getPasswordPolicy`**〔U2 as-built；原 `getSystemSettings` 為 R_SUPER-only 撞 403〕＋後端權威。created/updated 顯示語意分類（system/self/admin、僅管理員更新才顯示、不洩露 operator）。**零 migration**。
...
**Constraints**: 4 auth-only 端點〔含 U2 as-built getPasswordPolicy〕／2 窄寫 facade fn／**零 migration／零新 workspace crate／零 casbin seed**；`password_policy.rs` 喚醒（移除 `#![allow(dead_code)]`）；**MODAL-WIRING (g) amendment 已落（v1.3.0、⚠️ah、commit `63d35179`）**

**Scale/Scope**: 4 卡頁（+ 4 子元件）／4 端點 handler〔含 getPasswordPolicy〕 ＋ 2 facade fn ／`AS_BUILT_ROUTES` 50→54
...
| # | 檢查 | 裁定 |
|---|---|---|
| 1 | §I.1 base-web 權威（rust-api 缺 endpoint？）| ✅ PASS — 4 個新 self 端點前後端**同刀新增**、無缺口 |
...
**Post-Design 複查（Phase 1 後）**：設計未引入額外違反——2 窄寫 facade fn 守 facade-only＋§I.6 成對＋op-log redact；getProfile created/updated 語意解析不洩露 operator（不 join）；4 端點 auth-only（`AS_BUILT_ROUTES` 50→54、免 seed）；喚醒 024 原語（消費既測純函式）；前端 4 卡皆新檔/換佔位、循 (g)＋ADAPT/WRAPPER＋I18N-WIRING。**Gate 9/9 不變、可進 `/speckit-tasks`。**
...
├── quickstart.md        # 驗證流程
├── contracts/
│   ├── user-center-contract.md       # 4 auth-only 端點 + DTO + biz 碼 + created/updated + 024 消費
...
├── server/src/
│   ├── handler/user_center.rs                   # 新：4 auth-only handler（getProfile/updateProfile/changePassword）+ DTO + created/updated 語意 + 改密碼消費 024
...
│   └── tests/endpoint_coverage_lint.rs          # AS_BUILT_ROUTES 50→54（4 auth-only 路徑）
...
**Structure Decision**：沿用 worktree+submodule 結構（CLAUDE.md §1）。後端集中：1 新 handler 檔（4 端點）＋sys_user facade 加 2 窄寫 fn＋喚醒 password_policy；前端 1 頁換佔位＋4 子卡＋wrapper/typings/i18n。所有寫入走既有 `mutate_in_txn`+§I.6；讀端 `find_active_by_id`+`roles_of_user`。
