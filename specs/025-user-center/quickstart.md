...
> 交付＝個人中心 4 卡頁（基本资料〔含 created/updated 唯讀列〕/手机/邮箱/改密码）＋4 auth-only self 端點＋2 窄寫 facade fn＋喚醒 024 `password_policy`、零 migration。細節見 [`contracts/user-center-contract.md`](contracts/user-center-contract.md)、C-V 見 [`contracts/verification-commands.md`](contracts/verification-commands.md)。
...
## 1. 後端（4 auth-only 端點 + 2 窄寫 facade + 喚醒 024）
- `handler/user_center.rs`（getProfile/updateProfile/changePassword）；`sys_user` facade +`update_own_profile`/`change_own_password`；`password_policy.rs` 移除 `#![allow(dead_code)]`；main.rs +user_center router（`route_auth` 範式、無 require_policy）；`AS_BUILT_ROUTES` 50→54。
...
## 4. 三守恆 + 回歸 + build
- `entity_access_lint`（facade-only）／`endpoint_coverage_lint`（AS_BUILT 54）／migration up→down→up（零 migration 未破）＋`pnpm typecheck`。
