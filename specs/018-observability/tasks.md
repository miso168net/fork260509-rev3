...
- [ ] T013 [US2] rust enforce counter：`rust-api/server/src/auth/enforce.rs` `enforce_role_path_method` allow site（迴圈內 `return true`、~L123、命中即計）`metrics::counter!("casbin_enforce_total","decision"=>"allow").increment(1)`、deny site（函式尾 `false`、~L126、全 role 落空後）`…"deny"…`（★ 唯一 label decision、勿加 path/method/role；揮發行號以 `return true`/`false` 兩點為準；fail-closed DB-error 走 require_policy `?` 不計、research R3 取捨）
- [ ] T014 [US2] rust cleanup push：`rust-api/cleanup-job/src/main.rs` 加 `fn push_metrics(now_epoch:i64, deleted:u64)`（`PrometheusBuilder::new().install_recorder()` + `metrics::gauge!("cleanup_job_last_success_timestamp").set(...)`/`cleanup_job_rows_deleted` + `ureq::put("http://pushgateway:9091/metrics/job/cleanup_job").send_string(render)`、best-effort 只 warn 不改 exit；★ now_epoch 用 `std::time::SystemTime`（不引 chrono）；execute=rows_affected/dry-run=0）；★ 實碼 main.rs:41-63 為 execute/dry-run **互斥 if/else、無現成匯流點** → 先 hoist `let deleted:u64`（execute=`rows_affected`／dry-run=0）+ 成功旗標出兩分支、於匯流後單點呼叫 → worktree commit（U2 rust）＋ **bump rust-api pin**（§4.1）
...
- [ ] T016 [US2] `docker-compose.yml` services 加 `prometheus`(prom/prometheus:v3.12.0,`[metrics]`,卷 prometheus_data)／`postgres_exporter`(prometheuscommunity/postgres-exporter:v0.19.1,`[metrics]`,`DATA_SOURCE_PASS_FILE=/run/secrets/postgres_password` reuse、★ `DATA_SOURCE_URI=postgres:5432/soybean_admin_rust?sslmode=disable`〔DB 名 soybean_admin_rust、`DATA_SOURCE_USER=soybean`〕)／`redis_exporter`(oliver006/redis_exporter:**v1.85.0-alpine** ★,`[metrics]`,★ `REDIS_ADDR=redis://redis-stack:6379`〔redis 服務名＝redis-stack 非 redis〕,sh-wrapper `export REDIS_PASSWORD=$(cat /run/secrets/redis_password); exec /redis_exporter`)／`pushgateway`(prom/pushgateway:v1.11.3,`[metrics]`)；★ 4 service 皆 `networks: [rev3_net]`
