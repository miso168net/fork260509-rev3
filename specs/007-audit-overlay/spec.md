...
- **FR-004**: 系統 MUST 對 login 的**每個終端結果**（成功或任一失敗路徑）留**恰一筆**登入嘗試紀錄。〔**as-built 勘誤 (2026-06-28、021-login-lockout-redis-cache)**：此「每終端結果恰一筆」（exactly-one per terminal result）於 **Redis L1 負快取命中（已生效鎖定）之被壓制嘗試不再成立**——021 ②b 使鎖後嘗試早 return、不寫 `sys_login_attempt`（鎖後成本 O(1)、防分散式打單帳號的寫入放大灌爆稽核表）；**上鎖前歷程與「觸發鎖那一發」仍恰一筆**（走 L2/Redis-miss 路徑、019 FR-006 留存）。屬對 007 的有意識 as-built 演進、非違憲（§I.6 archetype B 只規範 append-only/不可竄改、未規範每嘗試必寫）。**註**：021 設計鏈（research D7/plan/tasks）原誤標此反轉為「019 FR-004」，實則 019 FR-004＝真實 IP 防偽（021 未碰）、exactly-one 之擁有者為本 007 FR-004。拍板＋as-built 詳 [DECISIONS §1（021）](../../docs/INTEGRATION-DECISIONS.md)。〕
...
- **SC-002**: login 每終端結果（成功＋各失敗）100% 留恰一筆登入嘗試紀錄、失敗筆 100% 帶真實來源 IP——由活體驗證證實。〔**as-built 勘誤 (2026-06-28、021)**：同 FR-004——021 鎖後 L1 命中之被壓制嘗試不寫列，故「每終端結果 100% 恰一筆」於鎖中壓制情形不再成立（量級訊號移觀測層 ②c 節流摘要 `target=security.lockout`→loki、非稽核表）；未鎖與觸發那發仍 100% 恰一筆。詳 [DECISIONS §1（021）](../../docs/INTEGRATION-DECISIONS.md)。〕
