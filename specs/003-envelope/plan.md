...
- **base-web（⚠️z `BASE-WEB-I18N-WIRING ★` 軌道、fork-delta `rev3-inline`）**：`app.d.ts` `Schema` 加 `backend` 型＋`translateBackendMsg` helper（自 `@/locales`）＋`locales/langs/{zh-cn,en-us}.ts` 加 `backend` 命名空間（13 固定碼 seed key 雙語）＋`service/request/index.ts` 兩翻譯點（`:71` modal、`:109` onError extraction）＋dedup companions（`:64`/`:51`）。
...
**Constraints**: 凍結 wire 契約忠實實作不重決（§I.3／DESIGN §7.3）；msg=去前綴語意 key（⚠️y）；**本刀無 biz endpoint**（`server` 僅 `main.rs`/health＋`.fallback`→`4040`）；base-web inline 嚴限 ⚠️z `BASE-WEB-I18N-WIRING ★` 三範圍、走 fork-delta `rev3-inline`；`4040`/`5003` msg 今日不顯示＝既認限制、本刀不修（R3）；push/merge 凍結至 finishing（§I.4／⚠️u、tasks 不得排 push）
...
*constitution-rev3 **v1.1.0**（含本刀觸發的 ⚠️z `BASE-WEB-I18N-WIRING ★` amendment）§IV 九題逐答——Phase 0 前評估＋Phase 1 後複評均 PASS：*
...
| 2 | 動 base-web inline？ | **YES→PASS**——i18n 接線改 `service/request/{index,shared}.ts`＋`locales/langs/*`＋`app.d.ts` Schema；屬**新授權 ⚠️z `BASE-WEB-I18N-WIRING ★` 軌道**三範圍 (i)/(ii)/(iii) 之內（constitution §III.2、v1.1.0）；走 fork-delta `rev3-inline`（修改型保留原行註解＋token、新增型標記圈界）。逐處 file:line 紀錄見 [contracts/i18n-key-convention.md](contracts/i18n-key-convention.md) |
...
| 6 | 抵觸 §II 拍板 #1~#13？ | **PASS**——受 ⚠️e/⚠️f/⚠️y 治理、無拍板需改；⚠️z amendment 為**新增** ★ 軌道（§V.3 MINOR）、未動 #1~#13 |
...
**Gate 結論：9/9 PASS（Q2/Q7 由 ⚠️z v1.1.0 amendment 授權滿足）；無 violation 待 justify、Complexity Tracking 不適用。**
...
└── base-web/                          # worktree（⚠️z BASE-WEB-I18N-WIRING ★、fork-delta rev3-inline）
    └── src/
...
**Structure Decision**：web（backend＋frontend）。rust-api＝純加 `server` crate 模組（無新 workspace crate）；base-web＝⚠️z 軌道三範圍 inline 接線。tests：rust in-crate `#[cfg(test)]`（bin-only 約束）＋base-web component。
...
- [contracts/i18n-key-convention.md](contracts/i18n-key-convention.md)：規約（4 根 common/auth/biz/system＋文法）＋13 固定碼 seed key 雙語表＋wire 去前綴/前端 `backend.` 前綴 (c)＋翻譯點 file:line＋⚠️z 軌道 fork-delta 紀錄＋typed-key cast
