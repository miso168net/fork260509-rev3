...
- Q: 未翻譯識別碼的 graceful fallback 顯示行為（終端使用者看到什麼）？ → A: B — 直接顯示原始 key 字串（vue-i18n 原生未命中行為、零額外 fallback 碼）；缺翻譯「大聲」暴露、易被抓修（rollout 期過渡態），不改顯通用訊息
...
錯誤訊息以**穩定、語言無關的識別碼（key）**交付、而非人話文字；前端將其翻譯成使用者當前語言並顯示；若某識別碼尚無翻譯，UI **graceful degrade**（顯示其原始 key 字串、絕不崩）。
...
3. **Given** 一個尚無翻譯的識別碼，**When** 顯示，**Then** UI 顯示該識別碼原始 key 字串（vue-i18n 原生行為）而不中斷
...
- **SC-006**: 未知識別碼 graceful degrade 達 100%——**顯示其原始 key 字串**（非空白、非崩潰、非通用替代訊息），component test 可斷言「顯示文字＝該 key」
