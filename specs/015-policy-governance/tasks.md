...
- [ ] T009 [US1] `updateRoleMenu` handler reload 觸發改：`if outcome.changed{load_policy}` → **`reload_and_publish` on Applied（Ok/非 Rejected、含空-diff）**（`rust-api/server/src/handler/system_manage.rs:1118-1127`）——§4.2 ③ 對齊、調整 011 reload-on-`changed`（D3；依 T003,T004；§4.2 ③⑤）
...
- [ ] T016 [US3] live gate 精準驗（in-crate `#[ignore]`、`--test-threads=1`）：撤含 protected Rejected→**不** `load_policy`、**不** publish（驗 publish 計數 or enforce 不變）／restore NoOp/NotFound→**不** reload／**空-diff** set_role_dimension（送相同 desired）Applied→**仍** reload+publish（§4.2 ③「不優化」、011-adjust 證）／真變更 Applied→reload+publish（C-V-3 gate 部分；依 T009,T007）
