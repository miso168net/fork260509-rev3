---
source_file: "fork260509-soybean-admin-docs/src/assets/VSCode调试指南01.png"
type: "image"
community: "Community 181"
tags:
  - graphify/image
  - graphify/EXTRACTED
  - community/Community_181
---

# VSCode 调试指南 Screenshot 01 — launch TS Debugger

## Connections
- [[TS Debugger launch configuration]] - `references` [EXTRACTED]
- [[VS Code Debugging Tutorial (debug.md)]] - `references` [EXTRACTED]
- [[VS Code breakpoint debugging workflow (4 annotated steps)]] - `rationale_for` [EXTRACTED]
- [[VSCode Debugging Guide 03 (breakpoint hit in about page onMounted)]] - `conceptually_related_to` [INFERRED]
- [[debug.ts sample script (transformToKebabCase + start)]] - `references` [EXTRACTED]

#graphify/image #graphify/EXTRACTED #community/Community_181