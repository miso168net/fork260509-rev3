---
source_file: "base-web/src/service/api/rev3-system-manage.ts"
type: "code"
community: "rev3 Menu Service Wrappers"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/rev3_Menu_Service_Wrappers
---

# fetchGetAccessLog (API access audit read)

## Connections
- [[AccessLogTable (API access audit table + filters + CSV export)]] - `calls` [EXTRACTED]
- [[Api.SystemManage.AccessLogList (PageRes wrapper)]] - `references` [EXTRACTED]
- [[Api.SystemManage.AccessLogSearchParams DTO (incl httpStatusClass)]] - `references` [EXTRACTED]
- [[pruneNullParams (strip unset filter params helper)]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/rev3_Menu_Service_Wrappers