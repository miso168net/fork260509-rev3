---
type: community
cohesion: 0.08
members: 33
---

# ok()

**Cohesion:** 0.08 - loosely connected
**Members:** 33 nodes

## Members
- [[add_role()]] - code - rust-api/server/src/handler/system_manage.rs
- [[batch_delete_menu()]] - code - rust-api/server/src/handler/system_manage.rs
- [[batch_delete_role()]] - code - rust-api/server/src/handler/system_manage.rs
- [[delete_menu()]] - code - rust-api/server/src/handler/system_manage.rs
- [[delete_role()]] - code - rust-api/server/src/handler/system_manage.rs
- [[delete_user()]] - code - rust-api/server/src/handler/system_manage.rs
- [[get_all_buttons()]] - code - rust-api/server/src/handler/system_manage.rs
- [[get_all_endpoints()]] - code - rust-api/server/src/handler/system_manage.rs
- [[get_all_pages()]] - code - rust-api/server/src/handler/system_manage.rs
- [[get_all_roles()]] - code - rust-api/server/src/handler/system_manage.rs
- [[get_menu_list_v2()]] - code - rust-api/server/src/handler/system_manage.rs
- [[get_menu_tree()]] - code - rust-api/server/src/handler/system_manage.rs
- [[get_role_button()]] - code - rust-api/server/src/handler/system_manage.rs
- [[get_role_endpoints()]] - code - rust-api/server/src/handler/system_manage.rs
- [[get_role_home()]] - code - rust-api/server/src/handler/system_manage.rs
- [[get_role_menu()]] - code - rust-api/server/src/handler/system_manage.rs
- [[normalize_endpoint_method()]] - code - rust-api/server/src/handler/system_manage.rs
- [[normalize_lowercase_and_mixed_to_upper()]] - code - rust-api/server/src/handler/system_manage.rs
- [[normalize_rejects_non_whitelisted()]] - code - rust-api/server/src/handler/system_manage.rs
- [[ok()]] - code - rust-api/server/src/handler/system_manage.rs
- [[reload_and_publish()]] - code - rust-api/server/src/handler/system_manage.rs
- [[reload_publish_roundtrip()]] - code - rust-api/server/src/handler/system_manage.rs
- [[reload_publish_roundtrip_binds_correct_channel()]] - code - rust-api/server/src/handler/system_manage.rs
- [[restore_menu()]] - code - rust-api/server/src/handler/system_manage.rs
- [[restore_policy()]] - code - rust-api/server/src/handler/system_manage.rs
- [[role_delete_guard()]] - code - rust-api/server/src/handler/system_manage.rs
- [[role_write_from_req()]] - code - rust-api/server/src/handler/system_manage.rs
- [[update_role()]] - code - rust-api/server/src/handler/system_manage.rs
- [[update_role_button()]] - code - rust-api/server/src/handler/system_manage.rs
- [[update_role_endpoints()]] - code - rust-api/server/src/handler/system_manage.rs
- [[update_role_home()]] - code - rust-api/server/src/handler/system_manage.rs
- [[update_role_menu()]] - code - rust-api/server/src/handler/system_manage.rs
- [[wire_id()]] - code - rust-api/server/src/handler/system_manage.rs

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/ok
SORT file.name ASC
```

## Connections to other communities
- 34 edges to [[_COMMUNITY_system_manage.rs]]
- 17 edges to [[_COMMUNITY_get_access_log()]]
- 4 edges to [[_COMMUNITY_add_ip_rule()]]
- 2 edges to [[_COMMUNITY_user_write_from_req()]]
- 2 edges to [[_COMMUNITY_menu_write_from_req()]]
- 1 edge to [[_COMMUNITY_sys_role.rs]]
- 1 edge to [[_COMMUNITY_auth.rs]]

## Top bridge nodes
- [[ok()]] - degree 44, connects to 6 communities
- [[wire_id()]] - degree 12, connects to 2 communities
- [[role_delete_guard()]] - degree 4, connects to 2 communities
- [[role_write_from_req()]] - degree 4, connects to 2 communities
- [[reload_and_publish()]] - degree 8, connects to 1 community