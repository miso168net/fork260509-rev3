---
source_file: "fork260509-soybean-admin-docs/src/guide/icon/intro.md"
type: "concept"
community: "Community 140"
tags:
  - graphify/concept
  - graphify/EXTRACTED
  - community/Community_140
---

# unplugin-icons (2)

## Connections
- [[Iconify]] - `references` [EXTRACTED]
- [[Iconify Icon Rendering (unplugin-icons)]] - `references` [EXTRACTED]
- [[Local SVG Icon Rendering]] - `references` [EXTRACTED]
- [[Local SVG Icon Rendering (vite-plugin-svg-icons, srcassetssvg-icon)]] - `references` [EXTRACTED]
- [[Offline Iconify Icon Loading]] - `semantically_similar_to` [INFERRED]
- [[System Icons Guide]] - `references` [EXTRACTED]

#graphify/concept #graphify/EXTRACTED #community/Community_140