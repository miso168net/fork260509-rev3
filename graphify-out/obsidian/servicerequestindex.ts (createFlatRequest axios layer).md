---
source_file: "base-web/src/service/request/index.ts"
type: "code"
community: "Community 5"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_5
---

# service/request/index.ts (createFlatRequest axios layer)

## Connections
- [[App.Service.Response (backend envelope type)]] - `references` [EXTRACTED]
- [[localesindex.ts ($t, setupI18n)]] - `calls` [EXTRACTED]
- [[serviceapi (barrel fetchGetAllRoles, fetchGetUserRoutes etc.)]] - `depends_on` [INFERRED]
- [[serviceapirev3-system-manage.ts (honest user wrappers)]] - `depends_on` [INFERRED]
- [[translate-backend-msg.spec.ts (i18n boundary test)]] - `semantically_similar_to` [INFERRED]
- [[translateBackendMsg (backend. prefix translation boundary)]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_5