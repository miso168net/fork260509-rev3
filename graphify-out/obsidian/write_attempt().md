---
source_file: "rust-api/server/src/model/facade/sys_login_attempt.rs"
type: "code"
community: "sys login attempt.rs"
location: "L242"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/sys_login_attemptrs
---

# write_attempt()

## Connections
- [[lockout_live_per_ip_count_isolation_and_d04()]] - `calls` [EXTRACTED]
- [[lockout_live_per_user_count_and_decide()]] - `calls` [EXTRACTED]
- [[lockout_live_sliding_window_recovery()]] - `calls` [EXTRACTED]
- [[lockout_live_sticky_gated_row_counted()]] - `calls` [EXTRACTED]
- [[login_attempt_active_model()]] - `calls` [EXTRACTED]
- [[sys_login_attempt.rs_1]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/sys_login_attemptrs