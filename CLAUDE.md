...
<!-- SPECKIT START -->
Active feature: **022-ip-access-control**（SDD 階段 1：spec ✅〔16/16 quality pass、**0 NEEDS CLARIFICATION**——brainstorm 已 firm 全部拍板〕；待 `/speckit-clarify`〔optional、可略〕→ `/speckit-plan`）。**通用 IP 白/黑名單存取控制閘**：全站每請求對 real_ip 比對 CIDR（**白名單放行 ＞ 黑名單阻擋 ＞ default-allow**）；DB `sys_ip_rule` 真相 → boot 載入 in-process（ArcSwap）→ redis invalidate 門鈴熱刷新；判定純記憶體 μs 級 **DoS-resilient**；**fail-OPEN**（§I.7）；寫端自鎖防護 + loopback/私網結構豁免；白名單**順便接 021 L0 seam 跳 lockout**（兌現 019 §4.2 per-IP 白名單 + 021 FR-012）；**admin 手動解鎖**（reset-marker per-dim、動 021 L2 `since` 單→雙拆分）；**CF Tunnel 繞 nginx 部署正式化 + 013 real_ip 窄 fallback 強化**。**1 migration**（新表 `sys_ip_rule`、破 rev3 連 4 刀 0-migration streak）+ 全域 middleware `ipgate_mw` + CRUD UI（含已刪/復原比照 menu、搜索分頁比照 user）。獨立 cold-review 接住 **1 HIGH**（CF-CIP 信任面收窄為窄 TUNNEL_ORIGIN+驗證）+ 3 MED（per-dim reset／blocked 零-DB 限未認證／riders 獨立驗收）已修進 brainstorm。021 ✅ 已收刀見 [MILESTONES `5e4f3228`](docs/INTEGRATION-MILESTONES.md)／[DECISIONS §1 ⚠️ad](docs/INTEGRATION-DECISIONS.md)。brainstorm=[docs/superpowers/022-ip-access-control.md]、spec=[specs/022-ip-access-control/spec.md]。
下一步: `/speckit-clarify`（optional、spec 已 0 NEEDS CLARIFICATION 可直接跳）→ `/speckit-plan`（產 plan/research/data-model/contracts；★ 新 workspace crate 無、但新 migration + 新 route ⇒ verification-commands 含 prod image build）→ `/speckit-tasks` → `/speckit-analyze` → 階段 2 `superpowers:executing-plans` + Workflow 驅動 TDD。設計接地見 brainstorm；4 執行單元雛形（U1 rust 地基〔013 窄 fallback+sys_ip_rule+ArcSwap〕／U2 rust 閘 ipgate_mw／U3 rust CRUD+解鎖／U4 base-web 管理頁）+ docs（CF 拓樸 DESIGN/DECISIONS/CLAUDE.md/CHECKLIST）。
<!-- SPECKIT END -->
