...
<!-- SPECKIT START -->
Active feature: **016-button-endpoint-policy（波 3 殿後刀＝Button-Endpoint 三維 RBAC runtime 編輯）** — SDD 全鏈✅（spec/plan/tasks/analyze）：spec✅（0 NEEDS CLARIFICATION、clarify 略過 full-coverage）／plan✅（Constitution 9/9 PASS、零 migration、零新 crate）／tasks✅（26 tasks：Setup1/Foundational1/US1×8/US2×8/US3×3/Polish5）／**analyze✅（3-reviewer 冷讀＋自審、0 CRITICAL、coverage 100%、endpoint-encoding 真碼驗證 CORRECT、remediation 已套：2 HIGH〔op-log 清理防 012 baseline RED／getRoleList UI-gateway 恢復走回收桶〕+3 MEDIUM+4 LOW）**、next **階段 2 `executing-plans`+Workflow**。把 011 `set_role_dimension`（menu）＋015 治理島延伸到 **button＋endpoint 兩維度**：button＝撿現成（updateRoleButton 直呼 `set_role_dimension(role,"button",codes)`、011+015 全免費繼承）；**endpoint＝新 `set_role_endpoints`（(path,method) 雙鍵 diff——因 endpoint=(v0=role,v1=path,v2=method) 真實 enforce 列、v2=method 變動套不上固定-v2 模型；治理 helper〔archive/protected-reject/同 txn 審計/reload_and_publish gate〕全復用 015）**。鎖出靠既有 15 protected endpoint seed（恢復路徑、零 migration、無硬鎖出）。回收桶三維由 v2-推導（'menu'/'button' 原值、HTTP method→'endpoint'、不改 archive_reason）。base-web MODAL-WIRING (c)（role-drawer button-auth un-mock＋endpoint-auth-modal 新建＋第三鈕）。〔015 已收 merge `65f1838`；詳 [DECISIONS §2](docs/INTEGRATION-DECISIONS.md)／CHECKLIST §2 波3〕
plan: `specs/016-button-endpoint-policy/plan.md`（**Constitution 9/9 PASS、零 migration、零新 crate**；research/data-model/contracts/quickstart 全產）。接地：6 編輯端點 policy（`m002:148-153` protected R_SUPER）＋button policy(16)＋endpoint policy(39、15 protected)＋button JSON(`sys_menu.buttons`)全波0已備、本刀只【註冊 6 route＋實作 handler＋新 set_role_endpoints＋bump AS_BUILT 37→43＋base-web】。6 單元 D1 button 寫端（reuse）／D2 endpoint facade（新 set_role_endpoints+鎖出）／D3 endpoint handler+route+AS_BUILT+回收桶 v2-推導+registry const／D4 base-web 三維 UI／D5 typings 收斂（最低優先、可拆）／D6 零回歸+2-instance+CDP。**4 拍板**：scope C（button+endpoint 完整）／un-protect 不做（延續 015 A）／回收桶 v2-推導／endpoint 鎖出靠既有 15 protected seed。pins 起點 rust-api `2ad2029`、base-web `0adfd12d`。
下一步: **階段 2 `superpowers:executing-plans` + Workflow 驅動**（tasks/analyze 已完；讀 tasks.md 26 task→分執行單元；每單元 implementer(TDD)→spec-compliance review→fix↔loop→code-quality review；rust serial／容器內 build·test〔live `--test-threads=1`〕／base-web `--no-verify`／**★ i18n 改後 restart base-web 才能 CDP 驗**／review 只讀／★絕不 push·merge until finishing；不用 `/speckit-implement`；§4.2 5 invariants 須自動化驗證 §8.8 DoD、C-V-2~8）。**★ 正確性關鍵**：endpoint 編輯動真實 (v0=role,v1=path,v2=method) 列、絕不用 v2='endpoint' 平行編碼（會不驅動 enforce）。**analyze remediation 已套**：H1 每 live 測 teardown 清自身 casbin op-log（trace 隔離、防 012 `t_cas==2` baseline RED）／H2 spec UI-gateway 端點恢復走回收桶＋T018 getRoleList scenario／M1 T001 精確 count／M2 registry 雙向 assertion／M3 restore 審計 raw-v2 by-design／L1 T021 typo 修。EXECUTE_COMMAND 不自動觸發、hooks 手動。
surface: **alt-login 4 流程 stub＝⚠️m 2026-06-22 延後出波3**（post-波3 v1-completeness slot、CHECKLIST §3.18）。波 4＝observability（obs-min log + metrics、`profiles:[obs]`/`[metrics]` opt-in、未開始）。016 接地揭露：6 編輯端點全 m002 已 seed（零 migration）／button 撿現成（set_role_dimension dimension-agnostic）／endpoint 需新雙鍵 facade／typings 收斂須 rev3-owned Omit+intersection 讀型（D5 可拆）。
<!-- SPECKIT END -->
