...
<!-- SPECKIT START -->
Active feature: **（無 active feature — 024-password-policy 已收刀 2026-07-02、merge `aca02c81`、feature branch 保留、pins rust-api `45c53e0`/base-web `ad661e9f`）**。024＝密碼複雜度政策（管理員可設定、系統驗證就緒；政策先行 2 刀組第 1 刀、刀2＝025-user-center 接原語 enforce 改密碼）：US1 admin 於 system-settings 設 7 政策〔m009 seed 2 number+5 enum:on,off KV＋前端 `NInputNumber` render 分支（:precision=0/:update-value-on-input=false）〕／US2 `validate_value_type` number 型驗證〔bare `"number"` **整串比對** 1..=1024、非法→2222 `biz.systemSettings.invalidValue`；★避 `split_once` 死碼、補 008 passthrough 缺口〕／US3 `auth/password_policy.rs` 純驗證原語〔`PasswordPolicy`+7 變體 `PolicyViolation`+`from_settings` 吃**非-entity** KV+`validate_password_complexity` 回全部違規非短路（chars().count() 含邊界、特殊符號 ascii_graphic&&!alnum、禁同帳號 eq_ignore_ascii_case）；本刀 dormant、零 `entity::`〕。收尾 UI：system-settings 滾動修復〔去 table 模板 `overflow-hidden`、改 layout main 滾〕+分區塊〔密碼策略/會話設定〕+2 欄緊湊 grid+2 區塊標題 i18n〔BASE-WEB-I18N-WIRING〕；m010（描述繁→簡）**已 revert**、描述維持繁體〔簡繁統一交未來 zh-TW locale feature〕。Constitution 9/9〔MODAL-WIRING (e) 涵蓋 ⚠️ag、user 親決 A、不 bump〕、final holistic review 全 PASS（含收尾階段 2 fresh reviewer）、rust 254 測+lint(2+2)+migration up→down→up+base-web typecheck+CDP（render/persist/scroll）全綠、prod build m009 baked-in、0 新 crate/0 新 route/0 新 wire。詳 [MILESTONES §1 `aca02c81`](docs/INTEGRATION-MILESTONES.md)／[DECISIONS §1 ⚠️ag](docs/INTEGRATION-DECISIONS.md)。
下一步: **下一刀待定（當前無 active feature）**。刀2 候選＝**025-user-center**（個人中心；接 024 `password_policy` 原語 enforce 改密碼、rev3 sys_user profile 欄已存缺 self-service 端點）。候選見 [DESIGN §8.4 roadmap](docs/INTEGRATION-DESIGN.md)、遞延見 [CHECKLIST §4.2／§3.I](docs/INTEGRATION-CHECKLIST.md)。另有登記：未來 **zh-TW locale feature**（建 zh-tw.ts+語言切換「中文」改「簡體」加「繁體」；024 刻意留簡繁混用待此處理）。
<!-- SPECKIT END -->
