...
<!-- SPECKIT START -->
Active feature: **無進行中**。**009-role-management ✅ 全綠收刀＋merge `113b681`（2026-06-16、波 2 第一刀＝Role 直刀、純 sys_role CRUD）**。波 0（001-007）＋波 1（008 User 刀）＋波 2 第一刀（009 Role 刀）全完成。
009 交付: 5 端點（getRoleList 分頁/模糊／add/update/delete/batchDelete）＋6 net-new facade fn＋**leaf 審計**（無 composite/redact、無 roles/password key）＋roleCode 不可變（update 不寫 code＋前端 `:disabled`）＋種子 id∈{1,2,3} 拒刪（單+批 all-or-nothing）＋dup→**2222**（pre-check＋`sql_err()` 23505 race catch、**永不 5000**、clarify A）＋5 route `enforce_mw`（讀=R_SUPER+R_ADMIN、寫=R_SUPER）＋`endpoint_coverage_lint` 6→11；**零 migration／零新 crate**；前端 +4 wrapper＋MODAL-WIRING **(a)**（(c) auth-modal OUT）。8 單元 subagent-driven TDD＋兩段式 review＋final holistic READY；C-V-0~8 全綠／10 SC／6 US／Constitution PASS。worktree rust `101e69d`/base-web `fdf1cc7e`、三 ref push origin、009 feature branch 保留。
下一步: **波 2 續刀＝Menu 刀**（序 Role→**Menu**→system_settings→審計讀端〔⚠️b 殿後〕）。Menu 需 net-new `sys_menu`＋migration（`getMenuTree`/`getAllPages`）；完成後解鎖 Role×Menu 授權指派（`getRoleMenu`/`updateRoleMenu`… 目前 m002 seeded-but-unimplemented）。→ Phase 0 brainstorm → 手動 `/speckit-specify`（`before_specify` pre-hook 建 feature branch）。
<!-- SPECKIT END -->
