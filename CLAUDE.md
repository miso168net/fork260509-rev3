...
> ⏳ **rev3 波 0~4 全完成＋019/020/021/022/023/024 收刀**（最近收刀 024-password-policy＝2026-07-02〔波4 後獨立刀＝密碼複雜度政策：admin 於 system-settings 設 7 政策〔m009 seed 2 number+5 enum＋number 型驗證整串比對〕＋`password_policy` dormant 純驗證原語〔供刀2 025-user-center enforce 改密碼〕＋收尾 UI〔滾動修復/分區塊/2 欄緊湊 grid/i18n〕、merge `aca02c81`〕；當前無 active feature）：infra/foundational／system-settings 打樋／data islands／治理島＋三維 RBAC runtime 編輯／**observability 觀測層**／**登入失敗節流 gate**／**角色刪除治理（020）**／**lockout redis 快取（021）**／**IP 存取控制閘（022）**／**列表欄位排序（023）**／**密碼複雜度政策（024）**（001-024）均已收刀，波次→刀對映詳 [MILESTONES §1](docs/INTEGRATION-MILESTONES.md)／[DECISIONS §2](docs/INTEGRATION-DECISIONS.md)、active snapshot 見 §6 marker。下一波 roadmap 見 [DESIGN §8.4](docs/INTEGRATION-DESIGN.md)、候選遞延見 [CHECKLIST §4.2／§3.I](docs/INTEGRATION-CHECKLIST.md)（rev2 研究三檔為史料、不移植不重作，見 §7.1）。
...
- **主線（Claude）只在每個單元邊界醒來 checkpoint**：復核 workflow 結果 ＋ **獨立 `git show --stat HEAD` 對該單元 commit 範圍**（implementer 自報的 filesChanged／gitClean **不可信**——曾報「5 檔乾淨」實 commit 7 檔〔偷加 `package.json` test script＋commit 了該放 `/tmp` 的測檔〕，必親自核對）＋ 對 load-bearing 單元**在 rust-api 容器內**（`docker exec`、host 無 toolchain）自跑 `cargo build`／`test` 自驗 ground-truth → bump 該單元 submodule SHA pin（§4.1）→ 啟下一支 workflow；全單元完成後跑整體 final holistic review。
- **Workflow script 撰寫三紀律**（silent-fail 防線）：① `agent()` 回傳必 **null-guard**——transient API error／user skip 會回 `null`，直接取 `.verdict` 會 throw 整支 workflow crash;回 `null` 即視為 inconclusive、retry 或跳過。② **reviewer prompt 必標明 in/out scope**——寫清「本單元範圍＝X、後續單元/波的 deferred 工作＝Y、不得 flag」;否則 reviewer 把下游 deferred 產物誤判 critical、又漏抓真 bug。③ **per-unit prompt 一律內聯進 script body 的 `const`、不靠 `args` 傳**——`args` 物件常被序列化成字串、script 內 `args.field` 變 `undefined`（silent、不 throw，agent 收到字面「undefined」task body）。
...
<!-- SPECKIT START -->
Active feature: **（無 active feature — 025-user-center 已收刀 2026-07-02、merge `ded99ee9`、feature branch 保留、pins rust-api `ef1d21a`/base-web `f3ad8e23`）**。025＝个人中心自助頁（政策先行 2 刀組第 2 刀＝收尾；MODAL-WIRING (g) v1.3.0 ⚠️ah 隨刀合入）：**4 auth-only self 端點**〔getProfile／updateProfile（**部分更新**、只 Set 有帶 Some 欄）／changePassword（固定順序 notFound→mismatch→oldMismatch→tooWeak、3 淨新 `biz.password.*` 碼）／**getPasswordPolicy**（★實作中新增第 4 端點：原計畫前端讀 super-only getSystemSettings 會 403 違 FR-009/FR-014、review loop 接住、7-key allowlist 不洩其他設定）；operator=claims.uid 不信 body id、`AS_BUILT_ROUTES` 50→**54**、免 casbin seed〕＋2 窄寫 facade fn〔`update_own_profile`/`change_own_password`、§I.6 成對+op-log redact+不套 with_roles+不撤 session §I.7〕＋**喚醒 024 dormant `password_policy`**〔刪 `#![allow(dead_code)]`、`find_all`→pairs→`from_settings`→`validate_password_complexity(policy,new,user_name)`〕。前端 4 塊狀 2 欄自助頁〔修改密码→邮箱→手机→基本资料、標題左+保存右、账号/角色/创建/修改时间**唯讀純文字**不上送、各區塊保存只送自己欄位、改密验证方式 radio（旧密码真改密／邮箱・手机验证码佔位→toast 建置中）、動態密碼 rule（讀 getPasswordPolicy）、US4 驗證控件佔位〕＋`Api.UserCenter` typings/wrapper/i18n 三檔同步。**created/updated 收尾拍板**（user 給渲染範例、推翻 adminUpdatedAt 僅-admin-顯示）：`updatedAt`（raw、null→「未修改」列一律顯示）＋`updatedBy`（system/self/admin 與 createdBy 共用 `classify_operator`、self 不標註、不洩 operator）。**修 2 latent bug**〔confirm rule 值快照 race→`toRef`（UI 改密間歇 silent 失敗、curl 測不到、CDP e2e+access_log forensics 定位）；切換验证方式清空 credential 防舊密明文殘顯〕。驗證全綠〔rust 255 測+lint(2+2)+typecheck+live curl/psql/CDP+prod build ×2+零回歸〕、final holistic 3 SHIP+U-polish focused 2 SHIP、Constitution 9/9、**0 migration/0 新 crate**。詳 [MILESTONES §1 `ded99ee9`](docs/INTEGRATION-MILESTONES.md)／[DECISIONS §1 ⚠️ah](docs/INTEGRATION-DECISIONS.md)。
下一步: **下一刀待定（當前無 active feature、政策先行 2 刀組全收）**。候選見 [DESIGN §8.4 roadmap](docs/INTEGRATION-DESIGN.md)、遞延見 [CHECKLIST §4.2／§3.I](docs/INTEGRATION-CHECKLIST.md)。另有登記：未來 **zh-TW locale feature**（建 zh-tw.ts+語言切換「中文」改「簡體」加「繁體」；024/025 刻意 zh-CN 為主待此處理）；**手機/信箱真實驗證**（025 US4/改密验证码為佔位、接 SMS/信箱服務屬未來）。
<!-- SPECKIT END -->
...
> dev 帳號由 rust-api migration（m001 建表＋m002 seed）產出、**已隨 002（rev2-schema-baseline）落地**。帳號如下：
...
- **〔ingress 拓樸註，022 起〕CF Tunnel 繞 nginx**（與上 3 種 compose 啟動模式**正交**、非另一 compose 模式）：cloudflared 直連 rust-api internal `:31081`（繞 front-nginx）＝第 3 ingress。rust-api 經 `TrustModel.tunnel` 窄信任（`deploy/trust-model.toml` 的 `tunnel=`、**只含 cloudflared origin、非整個內網**）+ `CF-Connecting-IP` fallback 還原真實 client real_ip（純函式 `apply_tunnel_fallback`、B2 反偽造：內網非 tunnel-origin 偽造不採信）。3 ingress 拓樸/信任邊界詳 [DESIGN §11.5](docs/INTEGRATION-DESIGN.md)／[DECISIONS §1 ⚠️ae](docs/INTEGRATION-DECISIONS.md)。★ footgun：tunnel origin 必須 ⊂ `internal_default`（否則 fallback 不觸發、見 `deploy/trust-model.toml` 註）。
...
>
> **WSL2 /mnt/d Edit 工具抖動（非 build 假綠、屬編輯側）**：在 `/mnt/d` drvfs 上用 Edit 改檔常報 `ENOENT statx …` 或「file modified since read」**但寫入多半已成功**——別當失敗而重做或腦補;關鍵 edit 後一律 `grep`／`sed -n` 回讀磁碟驗實況，逐一編輯、驗一個再下一個。
>
...
> - **base-web**：vite（`pnpm dev`）若沒熱載新加的 service fn（rev3 wrapper 在 `service/api/rev3-system-manage.ts`、view 走直接路徑 import、不經 barrel）→ vite 服務到該檔的 stale 版本（不含新 export）→ 瀏覽器丟 `SyntaxError: … does not provide an export named 'fetchXxx'`、該頁掛不起來、list API 完全不發。症狀出現即 `docker compose … restart base-web`（pnpm install 走卷快取、秒級就緒）。**同理 i18n locale 鍵 stale 變體**（019 C-V-7 實證）：新增 `backend.*.*` locale 鍵後 vite 也可能沒熱載新字典 → `$t('backend.x.y.z')` fallback 回 **key 本身** → toast/UI 顯 raw key（如 `backend.auth.login.locked`）而非譯文；★ 此變體**靜默無 error**（curl 看 2222+msg=key、`pnpm typecheck`、source grep 三者全綠都掩蓋、只有 CDP browser 實渲染 toast 抓得到＝curl≠modal 的 i18n 變體），同樣 `restart base-web` 後重載即在地化（committed 碼正確、fresh checkout／prod build 會把 locale 編進去）。故**任何加 i18n 鍵的 feature 跑 CDP toast 在地化驗收前先 `restart base-web`、且斷言 toast 文字非 raw key**。
