...
surface: 波 3 後續候選＝**Policy-governance 刀**（治理島 §4.2、復用 011 `set_role_dimension` DB-first＋archive/restore/PolicyMutated/**casbin enforcer 跨實例 pub-sub 接 014 已建 Redis 基建/watcher 範式**）／Button-Endpoint policy 縱切〔含 011 留 mock 的 button-auth-modal〕／⚠️m alt-login stub。014 攔截 3 真漏洞已修+驗（U2 TOCTOU SC-002／U4 used-grace SC-004／U4 同秒 `<=` fail-secure FR-007）。Redis sess pointer 快取按 data-model X-01 sanctioned defer。research.md D4 `<`→`<=` doc-drift 已校正。
<!-- SPECKIT END -->
