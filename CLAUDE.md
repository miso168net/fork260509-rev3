...
<!-- SPECKIT START -->
Active feature: **015-policy-governance（波 3 第二刀＝治理島 §4.2 policy governance 完成刀）** — SDD 階段 0 brainstorm✅（spec-design `docs/superpowers/015-policy-governance.md`）、next **user review spec-design → 手動 `/speckit-specify`**。把 011 `set_role_dimension` 寫側補成 §4.2 完整治理島：revoke→archive-move／restore（`getArchivedPolicies`+`restorePolicy`+NoOp）／PolicyMutated gate 完整版／跨實例 `casbin:policy:invalidate` pub-sub（復用 014 watcher）／policy-archive 回收桶 UI。**零 migration**（archive 表波0 m001 已備）。〔014 已收 merge `3066bc2`、op_log 測試債修 `7ec8fc3`、⚠️m alt-login 延後出波3；詳 [DECISIONS §2](docs/INTEGRATION-DECISIONS.md)／[MILESTONES](docs/INTEGRATION-MILESTONES.md)／CHECKLIST §3.18〕
plan: （未產出、待 `/speckit-specify`→`/speckit-plan`）。brainstorm spec-design 見 `docs/superpowers/015-policy-governance.md`（scope／§4.2 綱要／9 FR／8 SC／6 實作單元 L1-L6／拍板）。**1 拍板：A（user 親決 2026-06-22）un-protect/re-protect 不做＝protected 維持硬守門**（只鎖 19 種子核心 policy、custom 角色全可治理；做它須 §V.2 amend、YAGNI）。Out：button/endpoint 編輯（波3 ③）／archive purge／un-protect。
下一步: **user review `docs/superpowers/015-policy-governance.md`** → 手動 `/speckit-specify`（pre-hook 建 feature branch `015-policy-governance`、勿排進 brainstorm 觸發）→ `/speckit-clarify`（選配）→ `/speckit-plan`（Phase 0 research 紀律：grep `set_role_dimension`/`SetDimensionOutcome` 真簽名＋`sys_casbin_policy_archive` entity 13 欄＋`system_manage.rs` reload 觸發〔`changed`〕＋014 `spawn_settings_watcher` 形狀）→ `/speckit-tasks` → 階段 2 `executing-plans`+Workflow（不用 `/speckit-implement`；★ §4.2 5 invariants 須自動化驗證 §8.8 DoD）。
