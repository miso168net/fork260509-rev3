...
<!-- SPECKIT START -->
Active feature: **023-list-column-sort**（spec+clarify+plan 已產、feature branch `023-list-column-sort`；**阻擋於 MODAL-WIRING ★ (f) Amendment 待 user 親決**）。**列表欄位排序**：7 個分頁列表（user/role + 5 審計/治理頁、menu 排除）支援點欄頭 **server-side 多欄排序**〔點擊序＝優先序、naive-ui 原生 3-state 反序起手〕＋一鍵清除＋localStorage per-route 持久化。後端每端點 +單一 `sort` query 字串〔`field:dir,...`〕→ `parse_sort_spec`＋per-entity `match` 白名單〔防注入、非法→2222〕→ facade `list` 依序 order_by＋Id tie-break〔未指定逐列等同現況〕；匯出 3 審計頁共用 facade 自動反映排序（FR-016）。前端 `useTableSort` composable〔受控排序＋自維護點擊序＋持久化〕。資料層唯一改動＝index-only migration `m008`〔補 `idx_login_attempt_created_at`、clarify Q1 拍板取捨、推翻 0-migration〕。plan 見 [specs/023-list-column-sort/plan.md](specs/023-list-column-sort/plan.md)、Constitution Check 8/9 PASS〔唯 (f) gate〕。
下一步: **user 親決 MODAL-WIRING ★ (f) Amendment（§V.2）** → `/speckit-tasks` → `superpowers:executing-plans`（不用 /speckit-implement）。
<!-- SPECKIT END -->
