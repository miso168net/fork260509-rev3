...
<!-- SPECKIT START -->
Active feature: **（無 active feature；波 2 第三刀＝011-role-management ✅ 全完成、merge `b80c9e3` 2026-06-19、階段 0~2 全綠）** — 波 2 尚餘 **⚠️b 審計查詢讀端＋UI 刀**（殿後）尚未起手；待下一刀階段 0 `superpowers:brainstorming`
last（011-role-management as-built 詳帳見 [DECISIONS §2](docs/INTEGRATION-DECISIONS.md)、[MILESTONES §1](docs/INTEGRATION-MILESTONES.md) `b80c9e3`）: 角色 CRUD（getRoleList〔R_SUPER+R_ADMIN〕／add·update·delete·batch〔R_SUPER〕、delete guards 種子+使用中+自身、批次整批拒）＋**角色×選單授權**（getRoleMenu/updateRoleMenu＝**全專案首個 runtime casbin policy WRITE、DB-first**〔net-new `sys_casbin_rule::set_role_dimension` 於 `mutate_in_txn` 直寫 `entity::casbin_rule` 11-col＋原子 op-log＋②protected-reject＋寫後 `load_policy()` reload、**絕無 MgmtApi**——constitution §I.7 §4.2 ①②④⑤、B1 校正兌現〕、與 010 getUserRoutes 讀寫閉環）＋角色首頁（getRoleHome/updateRoleHome entity 寫原子）＋hasAuth `role:*`＋i18n 6 鍵。**9 端點**；3 執行單元 Workflow 驅動（U1 `2332002`→U2 `e3d828e`→U3 `caa1e4bc`）;C-V-0~11 全綠、holistic **SHIP**（10/10 SC、B1 零 MgmtApi grep-confirmed、2 nit 修 G1 role_home trace_id／N1 C-V-7 method）。**零 migration/entity/schema、無新 crate**；pins rust-api `c377444`→`30de791`／base-web `a59c2738`→`caa1e4bc`
下一步: **⚠️b 審計查詢讀端＋UI 刀**（波 2 殿後;三 log 讀端〔sys_operation_log/sys_access_log/sys_login_attempt〕＋R_SUPER policy seed＋manage 新頁 MODAL-WIRING (e)＋§5.8 讀端 filter；wire 從零〔rev2 零讀端〕；DESIGN §8.2、[DECISIONS §1 ⚠️b](docs/INTEGRATION-DECISIONS.md)）。待階段 0 `superpowers:brainstorming` 產出 `docs/superpowers/<NNN>-audit-*.md`
surface: ★ casbin 寫端 pattern（DB-first `set_role_dimension`）波3 Button/Endpoint policy 縱切＋policy-governance 治理刀復用（archive/restore/un-protect/PolicyMutated-優化/publish-watcher）；審計讀端首碰 op-log/access-log/login-attempt entity Model decode round-trip（CHECKLIST §3.4/§3.8 follow-up）＋op-log operation 封閉詞彙對齊；rev2 零讀端、無 mock 可鏡像、wire 從零
<!-- SPECKIT END -->
