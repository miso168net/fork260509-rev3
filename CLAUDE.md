...
<!-- SPECKIT START -->
Active feature: **020-role-delete-policy-archive**（修 P-011-1 HIGH；SDD 階段 1：spec/clarify/plan ✅、待 `/speckit-tasks`）。角色軟刪（單筆/批次）同一交易 archive-move 該 code **全維** casbin 授權（reuse `insert_archived`、reason=`role_soft_delete`）+ 自 `casbin_rule` 移除 → 重建同 code 不繼承；commit 後 archived>0 → `reload_and_publish`。回收桶（getArchivedPolicies）含這些列但 `restorable=false`、restorePolicy 拒復原（2222 `biz.policy.notRestorable`）。★ FR-005+FR-012 統一為**讀時 `created_at` 衍生**（restorable＝撤銷類 reason AND ∃ active 角色 code=v0 且 created_at<archived_at）＝**零 archive mutation／零 migration**（守 §I.6 archetype D）。2 執行單元：U1 rust（archive_all_role_policies helper + soft_delete/batch 擴充 + getArchivedPolicies 衍生 + restorePolicy 守門 + reload）／U2 base-web（policy-archive-table 條件復原鈕 + reason 標籤 + i18n，MODAL-WIRING/⚠️aa）。Constitution **9/9**、0 migration/crate/route。spec=[specs/020-role-delete-policy-archive/spec.md]、plan=[specs/020-role-delete-policy-archive/plan.md]。
下一步: `/speckit-tasks`（產 tasks.md）→ `/speckit-analyze` → 階段 2 `superpowers:executing-plans` + Workflow 驅動 TDD。設計接地見 plan.md／research.md／data-model.md／contracts/verification-commands.md（C-V-0~11）。
<!-- SPECKIT END -->
