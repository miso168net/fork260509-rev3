...
<!-- SPECKIT START -->
Active feature: **（無）** — 022-ip-access-control ✅ **已收刀**（2026-06-29、merge `d9c809e6`、feature branch 保留；rust-api `cb2767f`／base-web `aa2f57bc`、皆已推 fork remotes）。**通用 IP 白/黑名單存取控制閘**：全站每請求 `ipgate_mw` 比對 real_ip（**白名單放行 ＞ 黑名單阻擋〔reuse `PermissionDenied` 5003/403〕＞ default-allow**）；DB `sys_ip_rule`〔migration **m007**、破 rev3 連 4 刀 0-migration streak〕真相 → boot 載入 in-process `ArcSwap<RuleSet>`〔μs lock-free、DoS-resilient〕→ redis `ipgate:invalidate` 門鈴 watcher 熱刷新＋CRUD 本機即時 reload〔仿 015〕；全程 **fail-OPEN**（§I.7）。寫端自鎖防護＋loopback/私網結構豁免〔僅豁免阻擋、非 lockout-bypass〕＋白名單跳 021 lockout〔L0 seam、兌現 022-FR-004／019 §4.2／021 FR-012〕＋admin 手動解鎖〔reset-marker per-dim `since`〕＋**CF Tunnel 繞 nginx real_ip 窄 fallback**〔`apply_tunnel_fallback`：`base_conf==Fallback && peer∈TrustModel.tunnel`→採信 CF-Connecting-IP、B2 反偽造；★ footgun：tunnel origin 須 ⊂ internal_default〕＋3 ingress 拓樸正式化（DESIGN §11.5 勘誤「nginx 單一入口」）。6 route〔lint `AS_BUILT_ROUTES` 50／`ALL_ENDPOINT_POLICIES` 42〕＋base-web 管理頁〔list+CRUD+已刪復原+搜索分頁+手動解鎖 modal+i18n 6 鍵〕。U1-U4 執行單元＋U5 final-review polish〔Workflow 驅動 implementer→spec∥quality review→fix loop；主線逐單元邊界獨立自驗＋bump pin S9〕；**final holistic review 3/3 SHIP**〔0 critical/high/medium、12 LOW 皆驗證無害〕；prod image build 綠、migration up→down→up 綠、Constitution 9/9。詳 [MILESTONES `d9c809e6`](docs/INTEGRATION-MILESTONES.md)／[DECISIONS §1 ⚠️ae](docs/INTEGRATION-DECISIONS.md)。
下一步: **當前無 active feature**。下一刀候選見 [DESIGN §8.4 roadmap](docs/INTEGRATION-DESIGN.md)／[CHECKLIST §4.2／§3.I follow-up](docs/INTEGRATION-CHECKLIST.md)（rev2 研究三檔為史料、不移植不重作、見 §7.1）。
<!-- SPECKIT END -->
