...
<!-- SPECKIT START -->
Active feature: **018-observability**（波 4 observability、一刀三執行單元 obs-min→obs-full→dashboard）。Phase 0 brainstorm ✅ 完成、spec-design 落地 `676b13be`、**pending 手動 `/speckit-specify`**（feature branch 尚未建）。拍板：D1 一刀 018＋三單元（否決照搬 rev2 三刀）；D2 rev2 對等+hindsight、defer 同 rev2（非-root alloy／alert notification／DB log purge ⚠️n／pg_trgm）。act-on-code 接地：axum 0.7（axum-prometheus 0.7.0 相容）／log-side trace_id span 不存在→U1 含小 rust 單元（info_span request＋fmt().json()）／nginx `/api/metrics` 404＋JSON log 已就緒（不動）／`grafana_admin_password` DESIGN:706 宣稱 8 檔實 6 檔落差須補（U1）／metrics 全 greenfield／★MSRV 1.86 新 dep 必檢。spec-design＝[docs/superpowers/018-observability.md](docs/superpowers/018-observability.md)。
下一步: 手動 `/speckit-specify`（input＝`docs/superpowers/018-observability.md`）→ `before_specify` pre-hook 建 `018-observability` feature branch → clarify/plan/tasks/analyze（★ research.md 首要＝MSRV 1.86 新 dep〔axum-prometheus／metrics／pushgateway client〕對 1.86 檢）→ 階段 2 Workflow 驅動 TDD（U1 obs-min→U2 obs-full→U3 dashboard）。**不走 writing-plans**（§3 覆寫 skill 預設）。
<!-- SPECKIT END -->
