...
<!-- SPECKIT START -->
Active feature: **無（012-audit-log-query 已收刀 merge `55d34ff`、2026-06-19；★ 波 2 data islands 全完成）** — 波 2 四刀全收：009 User `07b67d2`／010 Menu `3810103`／011 Role `b80c9e3`／**012 Audit `55d34ff`（殿後刀）**。012＝審計查詢讀端＋Super-only 審計中心單頁三分頁（3 唯讀端點 getOperationLog/getAccessLog/getLoginAttempt R_SUPER GET）＋首個波 2 帶 migration 的刀（m005 delta：manage_audit menu+3 policy+menu policy+filter 索引、可逆、無新業務表、無 ALTER）。feature branch `012-audit-log-query` 保留（已 push）。
as-built（詳帳 [DECISIONS §2](docs/INTEGRATION-DECISIONS.md)）: m005 delta〔execute_unprepared 鏡像 m002/m001、sys_menu F3 partial-unique ON CONFLICT、up→down→up 可逆〕／3 facade list〔鏡像 sys_role::list、order_by_desc created_at,id、**★ IP 模糊 host(col)::text LIKE net-new**、文字 LOWER LIKE ESCAPE 非 ilike〕／net-new names_for_ids unfiltered enrich〔含已刪〕／handler §5.8 守門+日期 parse〔畸形→biz.audit.invalidDateRange 2222〕+parse_entity_id 守門+honest wire〔id number/IP 去 mask/nullable→Option 無 skip/payload Option<Value> 任意 JSON〕／main audit group R_SUPER／**lint[31→34]+全 migration glob seed 來源**〔涵蓋 m005、原僅讀 m002 會 FAIL〕／base-web 100% 淨新 3 fetch+pruneNullParams+3 honest typing〔**payload unknown｜null 非 Record**〕+NTabs 3 tab+op-log payload renderExpand+i18n／**★ biz key 分層**〔rust biz.* vs base-web backend.biz.*、攔截器補 backend.〕／**★ CDP 接住 entityId= 空字串 400 bug→parse_entity_id 後端守門**。C-V-0~9 全綠、唯讀無 op-log 寫（41→41、access-log +N=F1 基建）、prod build 編入、final holistic（fresh-agent）無 blocker（12 FR+9 SC+6 Edge 全覆蓋、cross-unit 接縫一致）。pins rust-api `30de791`→`fca64a0`／base-web `caa1e4bc`→`1764154e`。
下一步: **波 2 全完成 → 波 3（行為島＋policy、未開始）**：Auth/Token/Session 合刀／Policy-governance 刀／Button-Endpoint policy 縱切／（⚠️m 拍板後）alt-login stub。前置拍板 ⚠️m alt-login 入波排程（預設入波）。波 3 de-risk：治理島形狀宜先 spike（DESIGN §8.5）。button-auth-modal（011 留 mock）亦波 3。
surface: 012 遞延入後續刀＝D11 三欄 client_ip/xff_ip/real_ip IP forensic 模型+寫端 ALTER〔同 §3.11 XFF 完整化+Cloudflare〕／D4 §3.12 user 角色變更 payload delta／log retention（⚠️n）／pg_trgm fuzzy GIN〔scale〕／CSV 匯出／2xx-5xx 類別 quick-filter。CHECKLIST §3.7（access/login client_ip decode round-trip 經 012 entity Model 讀回已證）／§3.8（op-log operation 字串契約 012 已 exercise）已隨 012 觸發解決。
<!-- SPECKIT END -->
