...
<!-- SPECKIT START -->
Active feature: **012-audit-log-query（波 2 殿後刀＝審計查詢讀端＋UI、⚠️b）** — 階段 1 SDD 進行中：brainstorm ✅（`docs/superpowers/012-audit-log-query.md` `c7423d2`）／specify ✅（`spec.md`、17/17、0 NEEDS CLARIFICATION）／clarify ✅（0 ambiguity）／**plan ✅**（[`plan.md`](specs/012-audit-log-query/plan.md)＋research/data-model/contracts/quickstart、**Constitution 9/9 PASS**）／**tasks ✅**（[`tasks.md`](specs/012-audit-log-query/tasks.md)、28 tasks／7 US／~2 Workflow 單元）／**analyze ✅**（SHIP-WITH-NITS、9/9 PASS；獨立 reviewer 接住 F1-F5、remediation `6bbdd56`）；**SDD 設計鏈全完、待階段 2**（executing-plans + Workflow 驅動）。feature branch `012-audit-log-query`（local、未 push）。波 2 第三刀＝011 已收 `b80c9e3`
spec/拍板: 3 唯讀端點 getOperationLog/getAccessLog/getLoginAttempt（R_SUPER GET）+ Super-only 審計中心**單頁三分頁**（D1、/manage/audit、操作異動/API 存取/登入嘗試）。D4 §3.12 user 角色變更 payload delta 延後／D5 **m005 delta migration**（**首個波 2 migration**：manage_audit menu seed+3 讀端 policy+operation/access btree filter 索引、不動 frozen m002、合 ⚠️t）／**D11 client_ip/xff_ip/real_ip 三欄統一 IP forensic 模型延後**（讀端先行、012 讀現 schema〔op-log operator_ip／access·login client_ip〔=解析後〕+x_forwarded_for〕；IP 模型+寫端 ALTER〔005/007 audit infra、同 §3.11 XFF 完整化+Cloudflare CF-Connecting-IP〕留後續刀、brainstorm §11 sketch）／**D12 逐欄 fuzzy/exact**（文字/IP 模糊〔IP host()::text LIKE、operator by 名 resolve〕、enum/bool/數字精確下拉、created_at 範圍；4 子選擇預設可改）。§5.8+operator id→name unfiltered enrich／honest typings 從零（nullable→｜null）／純讀無 op-log 寫無 cleanup／lint [31→34]／無新 crate。pins 未動 rust-api `30de791`／base-web `caa1e4bc`
下一步: **階段 2** `superpowers:executing-plans` + **Workflow 驅動**（讀 tasks.md→依實際相依/獨立可審邊界分執行單元→U1 rust 讀端+m005／U2 base-web 審計中心頁；驅動提示詞見 §3）
surface: ★ m005＝首個波 2 migration（ALTER-free、僅 seed+btree 索引；模糊 LIKE seq-scan、pg_trgm 留 scale follow-up 不引 CREATE EXTENSION）；★ D11 後續刀＝ALTER 3 表+audit_ctx peer/CF capture（宜與 §3.11 一刀收）；100% 淨新 base-web 審計中心頁、wire 從零；收 §3.4（IP decode round-trip）/§3.8（operation 字串契約）；op-log entity_table='casbin_rule' 列＝011 policy 異動稽核軌；plan research 解：★ IP 模糊 `host()::text LIKE`＝net-new 無 codebase 先例〔U1 facade live 首驗〕／operator enrich＝net-new `sys_user::names_for_ids`〔unfiltered 含已刪〕／facade list 鏡像 sys_role::list〔page 0-based〕／m005 鏡像 m002 seed+m001 index〔execute_unprepared〕；★ analyze remediation（`6bbdd56`）：F1 access-log per-request 每 GET +1 列＝唯讀限 op-log〔非自審〕／F2 畸形日期→`backend.biz.audit.invalidDateRange` 2222〔僅起/僅訖寬鬆〕／F3 m005 sys_menu seed 須 `ON CONFLICT (route_name) WHERE deleted_at IS NULL`〔partial unique、bare 會 runtime error〕／F5 缺 view＝transform.ts throw 非 warn
<!-- SPECKIT END -->
