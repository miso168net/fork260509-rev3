...
<!-- SPECKIT START -->
Active feature: **（無）**。**020-role-delete-policy-archive ✅ 已收刀**（2026-06-27、修 P-011-1 HIGH、防角色重建繼承舊授權、merge `ebbd6c2c`、feature branch 保留）：角色軟刪（單筆/批次）同一交易 archive-move 該 code **全維** casbin 授權（reuse `insert_archived`、reason=`role_soft_delete`）+ 自 `casbin_rule` 移除 → 重建同 code **0 繼承**；archived>0 → `reload_and_publish`。三條繼承路徑全封：getMenu（archive-on-delete）／回收桶 restore（讀時 `created_at` 衍生 restorable、零 mutation／零 migration、守 §I.6 archetype D、統一 FR-005+FR-012）／**grant-during-delete FOR-UPDATE 鎖序（U1b＝user 拍板 A、四類 casbin 寫端〔delete/batch/restore/grant〕全先鎖 sys_role 列）**；restorePolicy 拒復原（2222 `biz.policy.notRestorable`）。3 執行單元 Workflow 驅動（U1 rust `46b2941`..`3c65d17`／U1b grant-lock `67ff2bd`／U2 base-web `fec0a6d`、pins outer `1e72d3fb`/`eafcbe90`）。驗證：rust 226 測（含 36 live）+ entity/endpoint lint + prod build + curl C-V-1/2/5/6/7/8 + CDP C-V-9 + fresh-agent holistic 冷讀全綠；Constitution **9/9**、0 migration/crate/route。詳 [DECISIONS §1 P-011-1](docs/INTEGRATION-DECISIONS.md)／[MILESTONES `ebbd6c2c`](docs/INTEGRATION-MILESTONES.md)。
下一步: 當前無 active feature。下一波 roadmap 見 [DESIGN §8.4](docs/INTEGRATION-DESIGN.md)、候選遞延見 [CHECKLIST §4.2／§3.I](docs/INTEGRATION-CHECKLIST.md)。
<!-- SPECKIT END -->
