...
│   ├── INTEGRATION-DESIGN.md              ← ★ 設計權威 / 核心事實（rev3 架構與執行順序 §7.2）
...
rev3 整合的核心 docs 階層（⏳ 尚未建立），內容由「研究歷史」→「設計權威」→「動態 todo」流動;**`INTEGRATION-DESIGN.md` 是核心事實**。
...
**此檔為核心事實**。參考 §7.1 三份檔重新設計 rev3 的架構與執行順序、Phase / feature 編排、拍板項、軌道定義、wire 不變式。
