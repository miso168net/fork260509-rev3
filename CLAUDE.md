...
<!-- SPECKIT START -->
Active feature: **（無 active feature — 015-policy-governance ✅ 已收 merge `65f1838` 2026-06-22）** — 波 3 第二刀＝Policy 治理島 §4.2 完成刀全綠收刀：把 011 `set_role_dimension` 寫側補成 §4.2 完整治理島＝revoke→archive-move（可復原、同 txn 原子）／restore 三態（Applied/NoOp/NotFound、created_at 跨表 coerce）／PolicyMutated gate（reload_and_publish on Applied 含空-diff、調整 011 reload-on-`changed`、user 確認）／跨副本 `spawn_policy_watcher`（`casbin:policy:invalidate`、復用 014）／回收桶 UI（MODAL-WIRING (e)）。**零 migration、零新 crate**。4 執行單元 Workflow 驅動（EU1 資料層 `5163d5a`／EU2 HTTP+gate `383515b`／EU3 watcher `2ad2029`／EU4 UI base-web `0adfd12d`）。C-V-1~7 全綠（含 2-instance 跨副本收斂+CLIENT KILL 重訂閱韌性／CDP 回收桶 UI 點復原+§3.13 消解／零回歸 22+146+prod build）、holistic READY_TO_MERGE。〔詳 [DECISIONS §2](docs/INTEGRATION-DECISIONS.md)／[MILESTONES](docs/INTEGRATION-MILESTONES.md)／CHECKLIST §2 波3〕
plan: （已收，無 active plan）。pins：rust-api `7ec8fc3`→`2ad2029`、base-web `f3b2bf07`→`0adfd12d`。1 拍板：**A un-protect/re-protect 不做＝protected 硬守門**（§4.2-faithful、做它須 §V.2 amend ②）。
下一步: **波 3 第三刀（殿後刀）＝Button-Endpoint policy 縱切** — runtime 三維授權編輯（純 policy、無新 entity、非行為島）、含 011 留 mock 的 button-auth-modal、**復用 015 完成的 `set_role_dimension`+archive+protected 治理島**。收完即波 3 全完成。**起手＝走 CLAUDE.md §3 階段 0 `superpowers:brainstorming`**（產 spec-design→`/speckit-specify` 手動起 feature branch）。
surface: **alt-login 4 流程 stub＝⚠️m 2026-06-22 延後出波3**（post-波3 v1-completeness slot、CHECKLIST §3.18;前端 3 表單完整但後端 4 全缺＋bind-wechat 空殼+真 OAuth、低 v1 價值;另揭 ⚠️c alova/captcha 完整包拍定但未落地）。波 4＝observability（obs-min log loki/alloy/grafana + metrics prometheus/exporter、`profiles:[obs]`/`[metrics]` opt-in、未開始）。
<!-- SPECKIT END -->
