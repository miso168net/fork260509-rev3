...
下一步: **`/speckit-tasks`**（產 tasks.md、dependency-ordered）→ `/speckit-analyze`（選配、跨檔一致性）→ 階段 2 `executing-plans`+Workflow（不用 `/speckit-implement`；★ §4.2 5 invariants 須自動化驗證 §8.8 DoD、C-V-2~5）。EXECUTE_COMMAND 不自動觸發、spec-kit hooks 手動處理。
