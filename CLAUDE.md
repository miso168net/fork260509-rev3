...
<!-- SPECKIT START -->
Active feature: **無**（**021-login-lockout-redis-cache ✅ 收刀**＝2026-06-28、硬化 019 登入鎖定：login gate 最前端加 **L1 Redis 負快取〔deny 層〕**堵分散式打單帳號的 DB 讀取/寫入放大、鎖後 O(1)；merge `5e4f3228`、feature branch 保留、rust-api `047a232`／outer pin `ec42bd4e`）。**D1** 雙維度 `lockout:ip`＋`lockout:user`（L2 達門檻依觸發維度 set_ex）／**D2/D3** 固定 TTL 900s 命中不 refresh 自癒／**②b** 鎖後 L1 命中不寫稽核＋**②c** 節流麵包屑（`security.lockout` suppressed=N→loki、≤1/60s/key）；**fail-OPEN** 退 L2 既有 DB gate（§I.7 §4.3）；**0 migration/crate/route/wire**、reuse 019 政策值 5/20/900、redis facade +`incr`/`take_suppressed`/`mark_locked`/`is_locked`。**T016 as-built 勘誤**：反轉 **007-audit-overlay FR-004/SC-002（exactly-one per terminal result）＋ 019 FR-008（gated sticky 審計痕跡那半）**、修正設計鏈原誤標「019 FR-004」（019 FR-004＝真實 IP 防偽、021 未碰；3 reviewer grep 實證）。帳號-DoS/IP 白名單/CAPTCHA 仍 019 §4.2 future（僅留 L0 seam、FR-012）。195 unit+8 live 測+活體 22 acceptance+3 reviewer holistic 全綠、Constitution 9/9。詳 [MILESTONES `5e4f3228`](docs/INTEGRATION-MILESTONES.md)／[DECISIONS §1 ⚠️ad](docs/INTEGRATION-DECISIONS.md)。
下一步: **當前無 active feature**。下一波 roadmap 見 [DESIGN §8.4](docs/INTEGRATION-DESIGN.md)；候選遞延見 [CHECKLIST §4.2／§3.I](docs/INTEGRATION-CHECKLIST.md)。新 feature 啟動＝階段 0 brainstorm（`superpowers:brainstorming`）→ 手動 `/speckit-specify`（見 §3 工作流）。
<!-- SPECKIT END -->
