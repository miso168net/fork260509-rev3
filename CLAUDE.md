...
<!-- SPECKIT START -->
Active feature: **011-role-management（波 2 第三刀＝Role）** — 階段 1 SDD 設計鏈：spec ✅／clarify ✅〔0 提問、3 brainstorm 拍板已入 spec Clarifications〕／plan ✅（[plan.md](specs/011-role-management/plan.md)）；待 `/speckit-tasks`
spec/plan: 角色 CRUD（getRoleList 分頁/filter〔R_SUPER+R_ADMIN〕／add·update·delete·batch〔R_SUPER〕）＋**角色×選單授權**（getRoleMenu/updateRoleMenu＝**全專案首次 casbin policy WRITE**〔MgmtApi `remove_filtered_policy`+`add_policies`、auto-persist、與 op-log **非原子**〕、與 010 getUserRoutes 讀寫閉環）＋角色首頁（getRoleHome/updateRoleHome entity 寫、原子）＋hasAuth gating（`role:*`）。**9 端點**（getRoleMenu/updateRoleMenu protected）。3 拍板：D1 menu-auth only〔button/endpoint policy 留波3〕／D2 治理姿態最小〔治理留波3、protected 不強制、op-log best-effort〕／D3 delete guards 種子+使用中+自身（hardcode seeded、零 migration）。**零 migration/entity/schema、無新 crate**（casbin 2.20.0 既有）；Constitution **9/9 PASS**（Q9 policy-island sequencing note：治理留波3）。wire id 域 number（roleId/menuIds 不轉 String、⚠️r）。pins rust-api `c377444`／base-web `a59c2738`（未動）
下一步: `/speckit-tasks`（產 tasks.md）→ `/speckit-analyze` → 階段 2 `superpowers:executing-plans`（~3 Workflow 單元：U1 Role CRUD〔delete guards〕／U2 Role×Menu+home〔casbin WRITE 讀寫閉環〕／U3 base-web〔menu-auth-modal 接線+hasAuth〕）
surface（plan-phase）: ★★ casbin policy 寫**非 txn-rollback 隔離**（adapter 自有連線 auto-persist）→ C-V-5/9 測必顯式 cleanup（snapshot+restore、psql 驗 casbin_rule 無殘留、勿污染 seed）；2 net-new facade helper（`count_users_by_role_id`／id↔code·route_name 映射）；button-auth-modal 留 mock（波3）；R_ADMIN 無 manage_role menu→getRoleList R_ADMIN access moot（忠實 seed）
<!-- SPECKIT END -->
