...
<!-- SPECKIT START -->
Active feature: **015-policy-governance（波 3 第二刀＝治理島 §4.2 policy governance 完成刀）** — SDD 全鏈✅（spec/plan/tasks/analyze；21 tasks、Constitution 9/9、analyze 3-reviewer 0 CRITICAL + remediation 已套）、next **階段 2 `executing-plans`+Workflow**。把 011 `set_role_dimension` 寫側補成 §4.2 完整治理島：revoke→archive-move／restore（`getArchivedPolicies`+`restorePolicy`+NoOp）／PolicyMutated gate 完整版／跨實例 `casbin:policy:invalidate` pub-sub（復用 014 watcher）／policy-archive 回收桶 UI。**零 migration**（archive 表波0 m001 已備）。〔014 已收 merge `3066bc2`、op_log 測試債修 `7ec8fc3`、⚠️m alt-login 延後出波3；詳 [DECISIONS §2](docs/INTEGRATION-DECISIONS.md)／[MILESTONES](docs/INTEGRATION-MILESTONES.md)／CHECKLIST §3.18〕
plan: `specs/015-policy-governance/plan.md`（**Constitution Check 9/9 PASS、零 migration、零新 crate**）。接地確認：archive 表（`m001:665`、13 欄）＋ 2 endpoint policy seed（getArchivedPolicies/restorePolicy、`m002:166-167`）＋ menu policy 全波0已備、本刀只【註冊 endpoint+實作+bump AS_BUILT 35→37】。6 單元 D1 archive-move（revoke HARD DELETE→snapshot+archive+delete 同 txn）／D2 restore+2 endpoint（NoOp/NotFound）／D3 PolicyMutated gate（**reload+publish on Applied 含空-diff、調整 011 reload-on-`changed` 對齊 §4.2**）／D4 `spawn_policy_watcher`（`casbin:policy:invalidate`、復用 014）／D5 base-web 回收桶頁（MODAL-WIRING (e)）／D6 零回歸（`role_menu_loop` teardown +archive 清）+2-instance。**1 拍板：A un-protect 不做＝protected 硬守門**（＝§4.2-faithful、做它須 §V.2 amend）。
下一步: **階段 2 `superpowers:executing-plans` + Workflow 驅動**（讀 tasks.md 21 task→分執行單元；每單元 implementer(TDD)→spec-compliance review→fix↔loop→code-quality review；rust serial／容器內 build·test〔live `--test-threads=1`〕／base-web `--no-verify`／review 只讀／★絕不 push·merge until finishing；不用 `/speckit-implement`；§4.2 5 invariants 須自動化驗證 §8.8 DoD、C-V-2~5）。**analyze 已驗：9/9 PASS、coverage 100%、0 CRITICAL；I1（015 改 011 reload-gate 對齊 §4.2-③）user 已確認；remediation 已套（T009 強調改011／T013 created_at-NULL／T016 menu-found-no-policy+publish-suppress）**。EXECUTE_COMMAND 不自動觸發、hooks 手動。
surface: 波 3 第三刀（015 後）＝**Button-Endpoint policy 縱切**（runtime 三維授權編輯、純 policy 無新 entity、復用 015 完成的 set_role_dimension+archive＋011 留 mock 的 button-auth-modal）。**alt-login 4 流程 stub＝⚠️m 2026-06-22 延後出波3**（post-波3 v1-completeness slot、CHECKLIST §3.18;另揭 ⚠️c alova/captcha 完整包拍定但未落地）。015 接地揭露：`sys_casbin_policy_archive` 波0已備（零 migration）／011 寫側已建（reload-on-`changed`、§4.2 gate 需 reconcile 空-diff reload）／014 Redis-watcher 範式可復用。Redis sess pointer 快取按 data-model X-01 sanctioned defer。
<!-- SPECKIT END -->
