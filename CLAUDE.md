...
<!-- SPECKIT START -->
Active feature: **無（013-xff-real-ip-forensics 已收刀 merge `8980d00`、2026-06-21；D11 遞延刀＝XFF→real_ip 鑑識、波 2 後獨立刀）** — 把 007 單值 XFF 解析重寫為**兩層信任模型**（peer-gate `Direct`／Tier-1 CDN 位置錨不硬 gate `CdnAnchored`／Tier-2 my-public+binding 走訪 `ProxyClean`/`ProxySoft`／`Fallback`）＋IIS 正規化＋CF-Connecting-IP overlay（`CdnVerified`/`CdnMismatch`、real_ip 不變）＋Cloudflare Tunnel；**四欄鑑識（peer/real/x_forwarded_for/ip_confidence 七態）× 三審計表全 4/4**（`m006` 可逆 delta）＋審計中心四欄顯示著色+三模糊一下拉篩＋nginx geo CF 閘（FR-010 防偽造）。feature branch `013-xff-real-ip-forensics` 保留。
as-built（詳帳 [DECISIONS §2](docs/INTEGRATION-DECISIONS.md)）: 解析核心 `audit_ctx.rs`〔normalize IIS-aware／resolve→(IpAddr,Confidence) 四分支／apply_cf_overlay top-level／Confidence 七態 as_str〕＋`TrustModel` TOML〔cdn/my_public/internal_default/bindings、per-set fail-safe、新 `toml` 0.8.23 parse-only dep〕；`m006` rename `client_ip→real_ip`/`operator_ip→operator_real_ip`+add peer/confidence/operator_*〔up→down→up+fresh chain 可逆、m001 凍結〕；**★ op-log 4/4 補完**＝`AuditOperator` 維持 Copy+net-new `AuditMeta` thread operator xff/confidence 過 ~15 facade（**user 親決補滿、反轉 D7「不動 16 facade」**）；honest wire 三 Item rename+add；base-web 四欄顯示 C2 序+NTag 七態著色〔共用 ip-confidence-tag.tsx〕+三模糊 ip_host_like/一下拉 eq C1+空字串守門+i18n〔confidence 七態 byte-identical rust↔d.ts↔i18n〕；nginx `geo $remote_addr`+map strip+proxy_set_header 覆寫〔FR-010 偽造中和 cdn_anchored≠cdn_mismatch〕+不啟 realip+零回歸既有 4 header＋trust-model CF cdn 啟用。6 執行單元 Workflow 驅動（U1`74f5c4d`→U2`493f015`+U2b`d4c1b74`→`c1fe429`／U3`25cab888`／U4 rust`627ebc6`+base-web`e79e7aa8`／U5 deploy`89e0fdf`／U6`3f2ebc6`〔清 IpForensics orphan〕）、每單元 implementer→spec+quality review＋主線獨立 ground-truth 自驗＋逐單元 pin。C-V-0~9 全綠（純測 36+12 案例/lint 2+2/m006 可逆+fresh temp-DB 全鏈/live 四欄 psql/curl 守門/CF cdn_verified·mismatch/Tunnel/FR-010 negative/**prod target image build**/零回歸），final holistic（fresh-agent）**PASS**（5 US+10 FR+7 SC 全覆蓋+跨單元接縫 byte-identical）。Constitution 9/9 PASS。pins rust-api `fca64a0`→`3f2ebc6`／base-web `1764154e`→`e79e7aa8`／deploy 直 outer。
下一步: **波 2＋D11(013) 全完成 → 波 3（行為島＋policy、未開始）**：Auth/Token/Session 合刀／Policy-governance 刀／Button-Endpoint policy 縱切／（⚠️m 拍板後）alt-login stub。前置拍板 ⚠️m alt-login 入波排程（預設入波）。波 3 de-risk：治理島形狀宜先 spike（DESIGN §8.5）。button-auth-modal（011 留 mock）亦波 3。
surface: 013 遞延入後續刀＝D4 user 角色變更 payload delta（§3.12）／log retention（⚠️n）／pg_trgm fuzzy GIN〔scale〕／CSV 匯出／2xx-5xx 類別 quick-filter／`operator_id` never-read warning（007 起 pre-existing、非 013 範圍）。012 §3.15 D11 IP forensic 模型（013 採四欄較 D11 原三欄更完整）＋§3.11 XFF 完整化已隨 013 兌現。
<!-- SPECKIT END -->
