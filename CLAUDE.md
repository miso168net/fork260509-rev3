...
<!-- SPECKIT START -->
Active feature: **021-login-lockout-redis-cache**（SDD 階段 1：spec ✅ committed `b154da6c`、checklist 16/16 PASS、0 NEEDS CLARIFICATION；待 `/speckit-clarify`〔optional、可略〕→ `/speckit-plan`）。硬化 019-login-lockout：對「已生效鎖定」加一層 **Redis 負快取（deny 層）**、DB 維持鎖定真相 → 堵分散式打單帳號下的 **DB 讀取/寫入放大**（per-user 無-LIMIT COUNT + 鎖中逐筆 INSERT 在 botnet 聚合下 → PG 飽和拖垮全系統）。**D1** 雙維度快取（`lockout:ip:`＋`lockout:user:`、依觸發維度寫；只記 ip 擋不住分散式 per-user）／**D2** 固定 TTL 900s 不 refresh（攻擊停 ~900s 自癒、不變永久鎖）／**D3** ②b 鎖中不逐筆寫稽核 + ②c 節流麵包屑（Redis `INCR`→loki log、非稽核表列、零 migration）。fail-open 退回 019 DB gate；reuse 019 政策值（5/20/900）；**0 migration/crate/route**、redis facade +`incr`。帳號-DoS/IP 白名單/CAPTCHA 切 019 §4.2 獨立 future（本刀僅留 L0 seam）。**有意識反轉 019 FR-004/FR-008**（鎖後不逐筆寫）。020 ✅ 已收刀見 [MILESTONES `ebbd6c2c`](docs/INTEGRATION-MILESTONES.md)／[DECISIONS §1 P-011-1](docs/INTEGRATION-DECISIONS.md)。brainstorm=[docs/superpowers/021-login-lockout-redis-cache.md]、spec=[specs/021-login-lockout-redis-cache/spec.md]。
下一步: `/speckit-clarify`（optional、spec 已 0 NEEDS CLARIFICATION 可略）→ `/speckit-plan`（產 plan＋research＋contracts；Phase 0 接地待固化見 brainstorm §8）→ `/speckit-tasks` → `/speckit-analyze` → 階段 2 `superpowers:executing-plans` + Workflow 驅動 TDD。
<!-- SPECKIT END -->
