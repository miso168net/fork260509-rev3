...
<!-- SPECKIT START -->
Active feature: **014-auth-token-session（波 3 第一刀＝Auth/Token/Session 合刀；SDD 進行中：brainstorm✅/specify✅/clarify✅〔零澄清〕/plan✅、next `/speckit-tasks`）** — 把 006 Auth 島最小段（login+enforce_mw+pointer+is_current stub）補成**完整行為島**：§4.1 token rotation chain（`/auth/refreshToken`+`decide_rotation` 純函式+reuse 偵測→撤整鏈→8888）＋§4.3 single-session（`resolve_policy` 讓全域 `single_session_default`×per-user `session_policy` 生效、`is_current` 補讀 policy→7777）＋硬即時撤銷（Redis denylist、停用/刪 user→access token 立即失效）＋cleanup-job binary（新 crate）＋2-instance 多副本驗證。feature branch `014-auth-token-session`。〔013 已收 merge `8980d00`，詳帳 DECISIONS §2／MILESTONES〕
plan: `specs/014-auth-token-session/plan.md`（spec/research/data-model/contracts 同目錄）。**★ 接地揭露 rust-api 零 Redis client → 本刀含建 Redis 基建**（憲法 §I.7「Redis 僅快取」假設它存在；denylist `revoked:user:{uid}`／pub-sub watcher `settings:invalidate`／pointer 快取 `sess:{uid}` 全靠它；用 Redis 避新表→守零 migration）。其餘接地：Claims 已備全欄〔iat/sid/jti/chain、006 前瞻〕、`is_current` 現況只比 pointer〔永遠踢、本刀補 resolve_policy〕、refresh/cleanup/denylist 全新建、base-web refresh 攔截器+7777/8888/3333 碼已配〔零改〕、session_policy UI 走 rev3-owned typing+MODAL-WIRING(a)、`sys_token` 9 欄齊無 jti欄〔jti 在 JWT〕。**Constitution 9/9 PASS**（零 migration item 8／§I.7 行為島 §4.1+§4.3 invariants 對齊 item 9；Complexity Tracking 登記 Redis 基建+cleanup-job 新 crate scope 實況）。4 拍板：C1 多實例 B-驗證／C2 撤銷 B-硬即時 fail-OPEN／C3 per-user session_policy UI 做／C4 alt-login 排除。7 實作單元（research §D）：Redis 基建→resolve_policy→rotation→denylist→cleanup-job→session_policy UI→多實例驗證。
下一步: **`/speckit-tasks`**（依賴序產 `tasks.md`）→ `/speckit-analyze`（選配跨檔 consistency）→ 階段 2 **`superpowers:executing-plans` + Workflow 驅動**（不用 `/speckit-implement`；7 單元 research §D、★ 每 §I.7 invariant 須自動化驗證 §8.8 DoD）。**user 可在 /speckit-tasks 前提：是否把「建 Redis 基建」拆獨立前置刀**（工程傾向建於首消費者本刀、見 plan Complexity Tracking）。
surface: 波 3 後續＝**Policy-governance 刀**（治理島 §4.2、復用 011 `set_role_dimension` DB-first＋archive/restore/PolicyMutated/**casbin enforcer 跨實例 pub-sub 接 014 Redis 基建**）／Button-Endpoint policy 縱切／⚠️m alt-login stub。013 遞延項見 CHECKLIST §3.16。Auth 島 §3.9 follow-up（token 即時撤銷/JWT 參數硬編）由 014 閉口。
<!-- SPECKIT END -->
