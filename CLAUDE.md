...
<!-- SPECKIT START -->
Active feature: **011-role-management（波 2 第三刀＝Role）** — 階段 1 SDD 設計鏈 ✅：spec／clarify／plan／tasks／analyze 全完；**analyze 抓出 B1 CRITICAL（casbin 寫違 constitution §I.7 §4.2）→ user 拍板 option 1 → 全設計鏈已 regen DB-first**（commit `ae3ae4d` spec/brainstorm、`8b1fd61` plan-set、`b1ca889` tasks）；待 階段 2
spec/plan: 角色 CRUD（getRoleList 分頁/filter〔R_SUPER+R_ADMIN〕／add·update·delete·batch〔R_SUPER〕）＋**角色×選單授權**（getRoleMenu/updateRoleMenu＝**全專案首次 casbin policy WRITE、DB-first**〔`sys_casbin_rule::set_role_dimension` 直寫 `entity::casbin_rule` 11-col 於 `mutate_in_txn` **原子** op-log、②protected-reject、寫後 `enforcer.load_policy()` reload、**絕不 MgmtApi**〕、與 010 getUserRoutes 讀寫閉環）＋角色首頁（getRoleHome/updateRoleHome entity 寫、原子）＋hasAuth gating（`role:*`）。**9 端點**（getRoleMenu/updateRoleMenu protected）。3 拍板：D1 menu-auth only／D2（★ B1 校正後）治理姿態＝**DB-first 合規**〔原子審計＋②protected enforce；archive/restore/protected-管理/PolicyMutated-優化/publish-watcher 治理機留波3〕／D3 delete guards 種子+使用中+自身。**零 migration/entity/schema、無新 crate**；Constitution **9/9 PASS**（Q9 DB-first 合規、§4.2 ①②④⑤ 滿足、僅治理功能波3）。wire id 域 number（⚠️r）。pins rust-api `c377444`／base-web `a59c2738`（未動）
下一步: 階段 2 `superpowers:executing-plans`（Workflow 驅動、~3 單元：U1 Role CRUD〔delete guards〕／U2 Role×Menu+home〔**DB-first set_role_dimension** 讀寫閉環＋原子審計＋②protected〕／U3 base-web〔menu-auth-modal 接線+hasAuth〕）
surface: ★ casbin 寫 **DB-first**（entity::casbin_rule 直寫、原子審計、②protected-reject、load_policy reload、絕不 MgmtApi——B1 教訓）；★★ 測 Role×Menu 必 cleanup（set_role_dimension commit、讀寫閉環需 committed 寫→snapshot+restore+psql 驗無殘留）；net-new facade `sys_casbin_rule`＋`count_users_by_role_id`；i18n 6 鍵（含 menuProtected）；button-auth-modal 留 mock（波3）；R_ADMIN 無 manage_role menu→getRoleList R_ADMIN access moot
<!-- SPECKIT END -->
