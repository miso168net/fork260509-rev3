...
<!-- SPECKIT START -->
Active feature: **022-ip-access-control**（SDD 階段 1：spec ✅〔16/16 quality pass、0 NEEDS CLARIFICATION〕／clarify ✅〔0 critical ambiguity、5 項 deferred 至 plan 已釘〕／plan ✅〔**Constitution 9/9**、產 research/data-model/contracts/quickstart〕／tasks ✅〔38 task、4 執行單元 U1-U4〕／analyze ✅〔**0 CRITICAL/HIGH**、2 獨立 reviewer 冷讀接住 **I1**〔tunnel-fallback 改新 pure fn `apply_tunnel_fallback`、不動 resolve_client_ip 簽名〕+ G1/G2/I2/D1/D2/T1，remediation 二輪已修〕；**SDD 設計鏈完成、待階段 2**）。**通用 IP 白/黑名單存取控制閘**：全站每請求對 real_ip 比對 CIDR（**白名單放行 ＞ 黑名單阻擋 ＞ default-allow**）；DB `sys_ip_rule` 真相 → boot 載入 in-process（ArcSwap）→ redis invalidate 門鈴熱刷新；判定純記憶體 μs 級 **DoS-resilient**；**fail-OPEN**（§I.7）；寫端自鎖防護 + loopback/私網結構豁免；白名單**順便接 021 L0 seam 跳 lockout**（兌現 019 §4.2 per-IP 白名單 + 021 FR-012）；**admin 手動解鎖**（reset-marker per-dim、動 021 L2 `since` 單→雙拆分）；**CF Tunnel 繞 nginx 部署正式化 + 013 real_ip 窄 fallback 強化**。**1 migration**（新表 `sys_ip_rule`、破 rev3 連 4 刀 0-migration streak）+ 全域 middleware `ipgate_mw` + CRUD UI（含已刪/復原比照 menu、搜索分頁比照 user）。獨立 cold-review 接住 **1 HIGH**（CF-CIP 信任面收窄為窄 TUNNEL_ORIGIN+驗證）+ 3 MED（per-dim reset／blocked 零-DB 限未認證／riders 獨立驗收）已修進 brainstorm。021 ✅ 已收刀見 [MILESTONES `5e4f3228`](docs/INTEGRATION-MILESTONES.md)／[DECISIONS §1 ⚠️ad](docs/INTEGRATION-DECISIONS.md)。brainstorm=[docs/superpowers/022-ip-access-control.md]、spec/plan=[specs/022-ip-access-control/]（plan.md＋research/data-model/contracts/quickstart）。★ plan 接地：arc-swap 已 Cargo.lock `1.9.1`〔transitive→提升 server direct dep、零版本 churn〕／TUNNEL_ORIGIN＝TrustModel 窄 `tunnel` 欄〔只 cloudflared origin、非 blanket internal、B2 收窄〕／migration **m007**／casbin seed R_SUPER + lint 44→50·36→42／blocked reuse `PermissionDenied`(5003/403)、零新碼。
下一步: **階段 2** `superpowers:executing-plans` + Workflow 驅動 TDD（貼 §3 驅動提示詞、`<NNN>-<feature-name>`＝`022-ip-access-control`）。設計接地見 tasks.md（T001-T038、4 執行單元、**pin bump 依單元邊界非 Phase**）／plan.md／research.md（D1-D11+5 deferred 釘死、**I1 tunnel-fallback＝新 pure fn 插 handler、不動 resolve_client_ip**）／contracts/verification-commands.md（C-V-0~14）。4 執行單元（U1 rust 地基〔013 窄 fallback + sys_ip_rule m007 + ArcSwap/watcher〕／U2 rust 閘 ipgate_mw〔CIDR/豁免/fail-OPEN/②c〕／U3 rust CRUD+手動解鎖〔per-dim since〕／U4 base-web 管理頁）+ docs（CF 拓樸 DESIGN/DECISIONS/CLAUDE.md/CHECKLIST）。★ 新 migration（m007）+ 新 route ⇒ verification-commands 含 prod image build。
<!-- SPECKIT END -->
