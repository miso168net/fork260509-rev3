...
<!-- SPECKIT START -->
Active feature: **012-audit-log-query（波 2 殿後刀＝審計查詢讀端＋UI、⚠️b）** — 階段 0 brainstorm ✅（`docs/superpowers/012-audit-log-query.md`、commit `c7423d2`、user 親決 5 拍板）；待**手動** `/speckit-specify` 進階段 1 SDD（pre-hook 建 `012-audit-log-query` feature branch）。波 2 第三刀＝011 已收 merge `b80c9e3`（as-built 見 [DECISIONS §2](docs/INTEGRATION-DECISIONS.md)）
...
下一步: 階段 1 **手動** `/speckit-specify`（input=brainstorm 文件；→ clarify/plan/tasks/analyze → 階段 2 executing-plans）
