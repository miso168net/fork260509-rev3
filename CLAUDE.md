...
<!-- SPECKIT START -->
Active feature: **無進行中 feature** — 014-auth-token-session（波 3 第一刀＝Auth/Token/Session 合刀）✅ **全完成並 merge `3066bc2`**（2026-06-22、feature branch 保留）。8 執行單元（U1 Redis 基建→U2 rotation+TOCTOU→U3 single-session→U4 denylist→U5 cleanup-job→U6 base-web UI→U7 2-instance→U8 holistic）全綠;詳帳見 [DECISIONS §2](docs/INTEGRATION-DECISIONS.md)／[MILESTONES](docs/INTEGRATION-MILESTONES.md)。
plan: （無進行中）。014 交付：§4.1 token rotation chain〔`/auth/refreshToken`+`decide_rotation`+TOCTOU lock-then-redecide 加固〕＋§4.3 single-session〔`resolve_policy`/`is_current` policy-aware 修「永遠踢」bug〕＋硬即時撤銷〔Redis denylist 撤 active+used、enforce_mw+refresh 雙查、同秒 `<=` fail-secure〕＋cleanup-job 新 crate〔prod 四處 COPY、binary 落地驗〕＋per-user session_policy UI＋2-instance 多副本驗證〔watcher CLIENT-KILL 重訂閱韌性〕。**零 migration、新 redis+cleanup-job crate、Constitution 9/9 PASS、final holistic PASS（5US/14FR/10SC/§I.7 全覆蓋）**。
下一步: **波 3 後續刀（未開始）** — 啟動走 CLAUDE.md §3 階段 0 `superpowers:brainstorming`（產 `docs/superpowers/<NNN>-<feature-name>.md`）→ 手動 `/speckit-specify`（pre-hook 建 feature branch）→ plan/tasks → 階段 2 `superpowers:executing-plans`+Workflow 驅動。
surface: 波 3 剩兩刀＝**Policy-governance 刀**（治理島 §4.2、復用 011 `set_role_dimension` DB-first＋archive/restore/PolicyMutated/**casbin enforcer 跨實例 pub-sub 接 014 已建 Redis 基建/watcher 範式**）→ Button-Endpoint policy 縱切〔含 011 留 mock 的 button-auth-modal、復用 set_role_dimension〕。**alt-login 4 流程 stub＝⚠️m 2026-06-22 重議→延後出波3**（user 親決 C、移 post-波3 v1-completeness slot、CHECKLIST §3.18;另揭 ⚠️c alova/captcha 完整包拍定但未落地）。014 攔截 3 真漏洞已修+驗（U2 TOCTOU SC-002／U4 used-grace SC-004／U4 同秒 `<=` fail-secure FR-007）。Redis sess pointer 快取按 data-model X-01 sanctioned defer。research.md D4 `<`→`<=` doc-drift 已校正。
<!-- SPECKIT END -->
